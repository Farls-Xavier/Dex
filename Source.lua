	...
	The main explorer interface
        ...
        local function main()
            ...
            Explorer.UpdateView = function()
                local maxNodes = math.ceil(treeFrame.AbsoluteSize.Y / 24)
                ...
                if oldSize ~= treeFrame.Size then
                    ...
                    renameBox.Size = UDim2.new(0, maxX - 100, 0, 20)
                ...
                local function move()
                    ...
                    for i = 1, #listEntries do
                        ...
                        if Lib.CheckMouseInGui(entry) then
                            ...
                            dragOutline.Size = UDim2.new(0, entry.Size.X.Offset - entry.Indent.Position.X.Offset, 0, 24)
            ...
            Explorer.NewListEntry = function(index)
                ...
                newEntry.Position = UDim2.new(0, 0, 0, 24 * (index - 1))
                newEntry.Size = UDim2.new(0, 250, 0, 24)

                local isRenaming = false
                local function animateExpandIcon(icon, isExpanded)
                    if not icon then return end
                    local target = isExpanded and 90 or 0
                    service.TweenService:Create(icon, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                ...
                newEntry.InputBegan:Connect(function(input)
                    ...
                    service.TweenService:Create(newEntry.Indent, TweenInfo.new(0.2), {
                ...
                newEntry.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        ...
                        releaseEvent = cloneref(game["Run Service"].Parent:GetService("UserInputService")).InputEnded
                        ...
                        mouseEvent = cloneref(game["Run Service"].Parent:GetService("UserInputService")).InputChanged
                        :Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.MouseMovement then
                ...
                newEntry.Indent.Expand.MouseButton1Down:Connect(function()
                    ...
                    animateExpandIcon(newEntry.Indent.Expand.Icon, expanded[node])
                    Explorer.Update()
                    Explorer.Refresh()
                end)
            ...
            Explorer.Refresh = function()
                local maxNodes = math.max(math.ceil((treeFrame.AbsoluteSize.Y) / 24), 0)
                ...
                for i = 1, maxNodes do
                    ...
                    if node then
                        ...
                        entry.Visible = true
                        entry.Position = UDim2.new(0, -scrollH.Index, 0, 24 * (i - 1))
                        entry.Size = UDim2.new(0, Explorer.ViewWidth, 0, 24)
                        ...
                        if selection.Map[node] then
                            entry.Indent.BackgroundColor3 = Color3.fromRGB(0, 162, 255)
                            entry.Indent.BorderSizePixel = 0
                            entry.Indent.BackgroundTransparency = 0.5
                        else
                            if Lib.CheckMouseInGui(entry) then
                                entry.Indent.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
                        ...
                        if #node > 0 and expanded[node] ~= 0 then
                            ...
                            entry.Indent.Expand.Icon.Rotation = expanded[node] and 90 or 0
            ...
            Explorer.SetupConnections = function()
                ...
                if Main.Elevated then
                    ...
                    descendantAddedCon = game["Run Service"].Parent.DescendantAdded:Connect(function(obj) pcall(
                        addObject, obj) end)
                    descendantRemovingCon = game["Run Service"].Parent.DescendantRemoving:Connect(function(obj) pcall(
                        removeObject, obj) end)
                end
            ...
            Explorer.ShowRightClick = function()
                ...
                if presentClasses["TouchTransmitter"] then context:AddRegistered("FIRE_TOUCHTRANSMITTER",
                        firetouchinterest == nil) end
                if presentClasses["ClickDetector"] then context:AddRegistered("FIRE_CLICKDETECTOR",
                        fireclickdetector == nil) end
                if presentClasses["ProximityPrompt"] then context:AddRegistered("FIRE_PROXIMITYPROMPT",
                        fireproximityprompt == nil) end
            ...
            Explorer.InitRightClick = function()
                ...
                local clth = function(str)
                    if str:sub(1, 28) == "game:GetService(\"Workspace\")" then str = str:gsub(
                        "game:GetService%(\"Workspace\"%)", "workspace", 1) end
                    if str:sub(1, 27 + #plr.Name) == "game:GetService(\"Players\")." .. plr.Name then str = str:gsub(
                        "game:GetService%(\"Players\"%)." .. plr.Name, "game:GetService(\"Players\").LocalPlayer", 1) end
                    return str
                end
                ...
                context:Register("FIRE_TOUCHTRANSMITTER",
                    {
                        ...
                        OnClick = function()
                            local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
                            if not hrp then return end
                            for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("TouchTransmitter") then
                                    firetouchinterest(hrp, v.Obj.Parent, 0) end end
                        end
                    })
                ...
                context:Register("FIRE_CLICKDETECTOR",
                    {
                        ...
                        OnClick = function()
                            local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
                            if not hrp then return end
                            for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ClickDetector") then
                                    fireclickdetector(v.Obj) end end
                        end
                    })
                ...
                context:Register("FIRE_PROXIMITYPROMPT",
                    {
                        ...
                        OnClick = function()
                            local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
                            if not hrp then return end
                            for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ProximityPrompt") then
                                    fireproximityprompt(v.Obj) end end
                        end
                    })
                ...
                context:Register("SELECT_LOCAL_PLAYER",
                    {
                        ...
                        Icon = 9,
                        OnClick = function()
                            pcall(function() if nodes[plr] then
...
return search]==]
            ...
            Explorer.InitEntryTemplate = function()
                entryTemplate = create({
                    { 1, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(0, 0, 0), BackgroundTransparency = 1, BorderColor3 = Color3.new(0, 0, 0), Font = Enum.Font.Gotham, Name = "Entry", Position = UDim2.new(0, 1, 0, 1), Size = UDim2.new(0, 250, 0, 20), Text = "", TextSize = 14, } },
                    { 2, "Frame",      { BackgroundColor3 = Color3.fromRGB(45, 45, 45), BackgroundTransparency = 1, BorderSizePixel = 0, Name = "Indent", Parent = { 1 }, Position = UDim2.new(0, 20, 0, 0), Size = UDim2.new(1, -20, 1, 0), } },
                    { 3, "UICorner",   { CornerRadius = UDim.new(0, 4), Parent = { 2 }, } },
                    { 4, "TextLabel",  { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "EntryName", Parent = { 2 }, Position = UDim2.new(0, 26, 0, 0), Size = UDim2.new(1, -26, 1, 0), Text = "Workspace", TextColor3 = Color3.fromRGB(220, 220, 220), TextSize = 13, TextXAlignment = 0, } },
                    { 5, "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, ClipsDescendants = true, Font = 3, Name = "Expand", Parent = { 2 }, Position = UDim2.new(0, -20, 0, 0), Size = UDim2.new(0, 20, 0, 20), Text = "", TextSize = 14, } },
                    { 6, "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://5642383285", ImageRectOffset = Vector2.new(144, 16), ImageRectSize = Vector2.new(16, 16), Name = "Icon", Parent = { 5 }, Position = UDim2.new(0, 2, 0, 2), ScaleType = 4, Size = UDim2.new(0, 16, 0, 16), } },
                    { 7, "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxasset://textures/ClassImages.png", ImageRectOffset = Vector2.new(304, 0), ImageRectSize = Vector2.new(16, 16), Name = "Icon", Parent = { 2 }, Position = UDim2.new(0, 4, 0, 2), ScaleType = 4, Size = UDim2.new(0, 16, 0, 16), } },
                })
            ...
            Explorer.Init = function()
                ...
                local explorerItems = create({
                    ...
                    { 2,  "Frame",      { BackgroundColor3 = Color3.fromRGB(30, 30, 30), BorderSizePixel = 0, Name = "ToolBar", Parent = { 1 }, Size = UDim2.new(1, 0, 0, 32), } },
                    { 3,  "Frame",      { BackgroundColor3 = Color3.fromRGB(20, 20, 20), BorderSizePixel = 0, Name = "SearchFrame", Parent = { 2 }, Position = UDim2.new(0, 6, 0, 4), Size = UDim2.new(1, -12, 0, 24), } },
                    { 4,  "TextBox",    { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, ClearTextOnFocus = false, Font = Enum.Font.Gotham, Name = "SearchBox", Parent = { 3 }, PlaceholderColor3 = Color3.fromRGB(100, 100, 100), PlaceholderText = "Search explorer...", Position = UDim2.new(0, 6, 0, 0), Size = UDim2.new(1, -30, 0, 24), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 13, TextXAlignment = 0, } },
                    { 5,  "UICorner",   { CornerRadius = UDim.new(0, 6), Parent = { 3 }, } },
                    { 6,  "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "Reset", Parent = { 3 }, Position = UDim2.new(1, -22, 0, 4), Size = UDim2.new(0, 16, 0, 16), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, } },
                    { 7,  "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://5034718129", ImageColor3 = Color3.fromRGB(150, 150, 150), Parent = { 6 }, Size = UDim2.new(0, 16, 0, 16), } },
                    ...
                    { 10, "Frame",      { BackgroundColor3 = Color3.fromRGB(20, 20, 20), BorderSizePixel = 0, Name = "ScrollCorner", Parent = { 1 }, Position = UDim2.new(1, -16, 1, -16), Size = UDim2.new(0, 16, 0, 16), Visible = false, } },
                    { 11, "Frame",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, ClipsDescendants = true, Name = "List", Parent = { 1 }, Position = UDim2.new(0, 0, 0, 32), Size = UDim2.new(1, 0, 1, -32), } },
                })
                ...
                local scrollBlockerV = createSimple("TextButton", {
                    Active = true,
                    AutoButtonColor = false,
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                ...
                scrollBlockerH.Parent = window.GuiElems.Content
                scrollBlockerV.Parent = window.GuiElems.Content
                ...
                for serviceName, serviceInstance in pairs(Services) do
                    insts[#insts + 1] = serviceInstance

                    local desc = getDescendants(serviceInstance)
                    for i = 1, #desc do
                        insts[#insts + 1] = desc[i]
                    end
                end
	...
	The main properties interface
        ...
        local function main()
            ...
            Properties.SoundPreviewProp = { Category = "Data", ValueType = { Name = "SoundPlayer" }, Class = "Sound", Name =
            "Preview", Tags = {} }
            ...
            Properties.StringToValue = function(prop, str)
                ...
                elseif typeName == "UDim2" then
                    ...
                    local xScale, xOffset, yScale, yOffset = tonumber(vals[1]), tonumber(vals[2]), tonumber(vals[3]),
                        tonumber(vals[4])
                    if xScale and xOffset and yScale and yOffset and #vals >= 4 then return UDim2.new(xScale, xOffset,
                            yScale, yOffset) end
                    ...
                    if showingAttrs and attrCount < maxAttrs then
                        ...
                        for name, val in pairs(attrs) do
                            ...
                            if not foundAttrs[name] then
                                ...
                                local attrProp = { IsAttribute = true, Name = "ATTR_" .. name, AttributeName = name, DisplayName =
                                name, Class = "Instance", ValueType = valType, Category = "Attributes", Tags = {} }
            ...
            Properties.NewPropEntry = function(index)
                ...
                nameFrame.Expand.InputBegan:Connect(function(input)
                    ...
                    local fullName = (prop.CategoryName and "CAT_" .. prop.CategoryName) or
                ...
                nameFrame.Expand.MouseButton1Down:Connect(function()
                    ...
                    animateExpandIcon(newEntry.NameFrame.Expand.Icon, expanded[fullName])
                    Properties.Update()
                    Properties.Refresh()
                end)
            ...
            Properties.Refresh = function()
                local maxEntries = math.max(math.ceil((propsFrame.AbsoluteSize.Y) / 24), 0)
                local maxX = propsFrame.AbsoluteSize.X
                ...
                for i = 1, maxEntries do
                    ...
                    if prop then
                        ...
                        entry.Visible = true
                        entry.Position = UDim2.new(0, -entryXOffset, 0, 24 * (i - 1))
                        entry.Size = UDim2.new(scaleType == 0 and 0 or 1,
                            scaleType == 0 and Properties.ViewWidth + valueWidth or 0, 0, 24)
                        ...
                        if prop.SpecialRow then
                            ...
                            local gName = (prop.CategoryName and "CAT_" .. prop.CategoryName) or
                            prop.Class .. "." .. prop.Name .. (prop.SubName or "")
                            ...
                            if prop.CategoryName then
                                ...
                                propNameBox.Font = Enum.Font.GothamBold
                                expand.Visible = true
                                propNameBox.TextColor3 = Color3.fromRGB(240, 240, 240)
                                ...
                                propNameBox.Font = Enum.Font.Gotham
                                entry.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
                                valueFrame.Visible = true

                                expand.Visible = typeData.Category == "DataType" and Properties.ExpandableTypes
                                [typeName] or Properties.ExpandableProps[gName]
                                ...
                                if propObj then
                                    if prop.IsAttribute then
                                        propCons[#propCons + 1] = getAttributeChangedSignal(propObj, prop.AttributeName)
                                        :Connect(function()
                                            Properties.DisplayProp(prop, i)
                                        end)
                                    else
                                ...
                                if gName == inputFullName then
                                    nameFrame.BackgroundColor3 = Color3.fromRGB(0, 162, 255)
                                    ...
                                    if typeData.Category == "Class" or typeData.Category == "Enum" or typeName == "BrickColor" then
                                        ...
                                        inputBox.Position = UDim2.new(scale, offset, 0, 24 * (i - 1))
                                        inputBox.Size = UDim2.new(1 - scale, -offset - endOffset - attributeOffset, 0, 24)
                            ...
                            if prop.CategoryName or Properties.ExpandableTypes[prop.ValueType and prop.ValueType.Name] or Properties.ExpandableProps[gName] then
                                ...
                                expand.Icon.Rotation = expanded[gName] and 90 or 0
                                expand.Visible = true
                            else
                        ...
                        pcall(function()
                            ...
                            if rootTypeName then
                                ...
                                elseif rootTypeName == "CFrame" then
                                    ...
                                    Vector3.new((subName == ".RightVector.X" and setVal) or rootRight.X,
                                ...
                                elseif rootTypeName == "PhysicalProperties" then
                                    ...
                                    (root and root.FrictionWeight) or rootProps.FrictionWeight
                                    local elasticityWeight = (subName == ".ElasticityWeight" and setVal) or
                                    (root and root.ElasticityWeight) or rootProps.ElasticityWeight
                                ...
                                elseif rootTypeName == "Ray" then
                                    ...
                                    Vector3.new((subName == ".Direction.X" and setVal) or rootDirection.X,
            ...
            Properties.InitInputBox = function()
                inputBox = create({
                    { 1, "Frame",    { BackgroundColor3 = Color3.fromRGB(20, 20, 20), BorderSizePixel = 0, Name = "InputBox", Size = UDim2.new(0, 200, 0, 24), Visible = false, ZIndex = 2, } },
                    { 2, "UICorner", { CornerRadius = UDim.new(0, 4), Parent = { 1 }, } },
                    { 3, "TextBox",  { BackgroundColor3 = Color3.new(0, 0, 0), BackgroundTransparency = 1, BorderSizePixel = 0, ClearTextOnFocus = false, Font = Enum.Font.Gotham, Parent = { 1 }, PlaceholderColor3 = Color3.fromRGB(100, 100, 100), Position = UDim2.new(0, 6, 0, 0), Size = UDim2.new(1, -12, 1, 0), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 13, TextXAlignment = 0, ZIndex = 2, } },
                })
            ...
            Properties.InitEntryStuff = function()
                Properties.EntryTemplate = create({
                    { 1,  "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.fromRGB(35, 35, 35), BorderColor3 = Color3.fromRGB(45, 45, 45), Font = Enum.Font.Gotham, Name = "Entry", Position = UDim2.new(0, 1, 0, 1), Size = UDim2.new(0, 250, 0, 24), Text = "", TextSize = 14, } },
                    { 2,  "UICorner",   { CornerRadius = UDim.new(0, 4), Parent = { 1 }, } },
                    { 3,  "Frame",      { BackgroundColor3 = Color3.fromRGB(0, 162, 255), BackgroundTransparency = 1, BorderSizePixel = 0, Name = "NameFrame", Parent = { 1 }, Position = UDim2.new(0, 20, 0, 0), Size = UDim2.new(1, -40, 1, 0), } },
                    { 4,  "TextLabel",  { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "PropName", Parent = { 3 }, Position = UDim2.new(0, 4, 0, 0), Size = UDim2.new(1, -4, 1, 0), Text = "Anchored", TextColor3 = Color3.fromRGB(200, 200, 200), TextSize = 13, TextTruncate = 1, TextXAlignment = 0, } },
                    { 5,  "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, ClipsDescendants = true, Font = 3, Name = "Expand", Parent = { 3 }, Position = UDim2.new(0, -20, 0, 2), Size = UDim2.new(0, 20, 0, 20), Text = "", TextSize = 14, Visible = false, } },
                    { 6,  "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://5642383285", ImageRectOffset = Vector2.new(144, 16), ImageRectSize = Vector2.new(16, 16), Name = "Icon", Parent = { 5 }, Position = UDim2.new(0, 2, 0, 2), ScaleType = 4, Size = UDim2.new(0, 16, 0, 16), } },
                    { 7,  "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = Enum.Font.Gotham, Name = "ToggleAttributes", Parent = { 3 }, Position = UDim2.new(1, -85, 0, 0), Size = UDim2.new(0, 85, 0, 24), Text = "[SETTING: OFF]", TextColor3 = Color3.new(1, 1, 1), TextSize = 13, Visible = false, } },
                    { 8,  "Frame",      { BackgroundColor3 = Color3.fromRGB(30, 30, 30), BackgroundTransparency = 0, BorderSizePixel = 0, Name = "ValueFrame", Parent = { 1 }, Position = UDim2.new(1, -120, 0, 2), Size = UDim2.new(0, 115, 1, -4), } },
                    { 9,  "UICorner",   { CornerRadius = UDim.new(0, 4), Parent = { 8 }, } },
                    { 10, "Frame",      { BackgroundColor3 = Color3.fromRGB(50, 50, 50), BorderSizePixel = 0, Name = "Line", Parent = { 8 }, Position = UDim2.new(0, -1, 0, 0), Size = UDim2.new(0, 1, 1, 0), } },
                    { 11, "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "ColorButton", Parent = { 8 }, Size = UDim2.new(0, 20, 0, 24), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, Visible = false, } },
                    { 12, "Frame",      { BackgroundColor3 = Color3.new(1, 1, 1), BorderColor3 = Color3.new(0, 0, 0), Name = "ColorPreview", Parent = { 11 }, Position = UDim2.new(0, 5, 0, 7), Size = UDim2.new(0, 10, 0, 10), } },
                    { 13, "UIGradient", { Parent = { 12 }, } },
                    { 14, "Frame",      { BackgroundTransparency = 1, Name = "EnumArrow", Parent = { 8 }, Position = UDim2.new(1, -16, 0, 4), Size = UDim2.new(0, 16, 0, 16), Visible = false, } },
                    { 15, "Frame",      { BackgroundColor3 = Color3.fromRGB(200, 200, 200), BorderSizePixel = 0, Parent = { 14 }, Position = UDim2.new(0, 8, 0, 9), Size = UDim2.new(0, 1, 0, 1), } },
                    { 16, "Frame",      { BackgroundColor3 = Color3.fromRGB(200, 200, 200), BorderSizePixel = 0, Parent = { 14 }, Position = UDim2.new(0, 7, 0, 8), Size = UDim2.new(0, 3, 0, 1), } },
                    { 17, "Frame",      { BackgroundColor3 = Color3.fromRGB(200, 200, 200), BorderSizePixel = 0, Parent = { 14 }, Position = UDim2.new(0, 6, 0, 7), Size = UDim2.new(0, 5, 0, 1), } },
                    { 18, "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "ValueBox", Parent = { 8 }, Position = UDim2.new(0, 6, 0, 0), Size = UDim2.new(1, -12, 1, 0), Text = "", TextColor3 = Color3.fromRGB(230, 230, 230), TextSize = 13, TextTruncate = 1, TextXAlignment = 0, } },
                    { 19, "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = Enum.Font.GothamBold, Name = "RightButton", Parent = { 8 }, Position = UDim2.new(1, -20, 0, 0), Size = UDim2.new(0, 20, 0, 24), Text = "...", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, Visible = false, } },
                    { 20, "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "SettingsButton", Parent = { 8 }, Position = UDim2.new(1, -20, 0, 0), Size = UDim2.new(0, 20, 0, 24), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, Visible = false, } },
                    { 21, "Frame",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Name = "SoundPreview", Parent = { 8 }, Size = UDim2.new(1, 0, 1, 0), Visible = false, } },
                    { 22, "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "ControlButton", Parent = { 21 }, Size = UDim2.new(0, 20, 0, 24), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, } },
                    { 23, "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://5642383285", ImageRectOffset = Vector2.new(144, 16), ImageRectSize = Vector2.new(16, 16), Name = "Icon", Parent = { 22 }, Position = UDim2.new(0, 2, 0, 4), ScaleType = 4, Size = UDim2.new(0, 16, 0, 16), } },
                    { 24, "Frame",      { BackgroundColor3 = Color3.fromRGB(80, 80, 80), BorderSizePixel = 0, Name = "TimeLine", Parent = { 21 }, Position = UDim2.new(0, 26, 0.5, -1), Size = UDim2.new(1, -34, 0, 2), } },
                    { 25, "Frame",      { BackgroundColor3 = Color3.fromRGB(150, 150, 150), BorderSizePixel = 0, Name = "Slider", Parent = { 24 }, Position = UDim2.new(0, -4, 0, -8), Size = UDim2.new(0, 8, 0, 16), } },
                    { 26, "UICorner",   { CornerRadius = UDim.new(0, 2), Parent = { 25 }, } },
                    { 27, "TextButton", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "EditAttributeButton", Parent = { 1 }, Position = UDim2.new(1, -20, 0, 0), Size = UDim2.new(0, 20, 0, 24), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, } },
                    { 28, "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://5034718180", ImageTransparency = 0.20000000298023, Name = "Icon", Parent = { 27 }, Position = UDim2.new(0, 2, 0, 4), Size = UDim2.new(0, 16, 0, 16), } },
                    { 29, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.fromRGB(45, 45, 45), BorderSizePixel = 0, Font = Enum.Font.GothamMedium, Name = "RowButton", Parent = { 1 }, Size = UDim2.new(1, 0, 1, 0), Text = "Add Attribute", TextColor3 = Color3.new(1, 1, 1), TextSize = 13, Visible = false, } },
                    { 30, "UICorner",   { CornerRadius = UDim.new(0, 4), Parent = { 29 }, } },
                })
            ...
            Properties.Init = function() -- TODO: MAKE BETTER
                local guiItems = create({
                    ...
                    { 2,  "Frame",      { BackgroundColor3 = Color3.fromRGB(30, 30, 30), BorderSizePixel = 0, Name = "ToolBar", Parent = { 1 }, Size = UDim2.new(1, 0, 0, 32), } },
                    { 3,  "Frame",      { BackgroundColor3 = Color3.fromRGB(20, 20, 20), BorderSizePixel = 0, Name = "SearchFrame", Parent = { 2 }, Position = UDim2.new(0, 6, 0, 4), Size = UDim2.new(1, -12, 0, 24), } },
                    { 4,  "TextBox",    { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, ClearTextOnFocus = false, Font = Enum.Font.Gotham, Name = "SearchBox", Parent = { 3 }, PlaceholderColor3 = Color3.fromRGB(100, 100, 100), PlaceholderText = "Search properties...", Position = UDim2.new(0, 6, 0, 0), Size = UDim2.new(1, -30, 0, 24), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 13, TextXAlignment = 0, } },
                    { 5,  "UICorner",   { CornerRadius = UDim.new(0, 6), Parent = { 3 }, } },
                    { 6,  "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "Reset", Parent = { 3 }, Position = UDim2.new(1, -22, 0, 4), Size = UDim2.new(0, 16, 0, 16), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, } },
                    { 7,  "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://5034718129", ImageColor3 = Color3.fromRGB(150, 150, 150), Parent = { 6 }, Size = UDim2.new(0, 16, 0, 16), } },
                    { 8,  "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(0.12549020349979, 0.12549020349979, 0.12549020349979), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "Refresh", Parent = { 2 }, Position = UDim2.new(1, -20, 0, 1), Size = UDim2.new(0, 18, 0, 18), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, Visible = false, } },
	...
	A script viewer that is basically a notepad
        ...
        local function main()
            ...
            ScriptViewer.ViewScript = function(scr)
                ...
                if not success or not source then source, PreviousScr = "-- DEX - Source failed to decompile", nil else PreviousScr =
                    scr end
            ...
            ScriptViewer.Init = function()
                ...
                codeFrame.Frame.Position = UDim2.new(0, 0, 0, 32)
                codeFrame.Frame.Size = UDim2.new(1, 0, 1, -32)
                codeFrame.Frame.Parent = window.GuiElems.Content

                -- Header Buttons
                local copy = create({
                    { 1, "TextButton", { BackgroundColor3 = Color3.fromRGB(45, 45, 45), BorderSizePixel = 0, Font = Enum.Font.Gotham, Name = "Copy", Parent = window.GuiElems.Content, Position = UDim2.new(0, 5, 0, 4), Size = UDim2.new(0, 140, 0, 24), Text = "Copy to Clipboard", TextColor3 = Color3.new(1, 1, 1), TextSize = 12, } },
                    { 2, "UICorner",   { CornerRadius = UDim.new(0, 6), Parent = { 1 }, } },
                })
                Lib.ButtonAnim(copy,
                    { Mode = 2, StartColor = Color3.fromRGB(45, 45, 45), LerpTo = Color3.fromRGB(0, 162, 255), LerpDelta = 0.15 })
                ...
                local save = create({
                    { 1, "TextButton", { BackgroundColor3 = Color3.fromRGB(45, 45, 45), BorderSizePixel = 0, Font = Enum.Font.Gotham, Name = "Save", Parent = window.GuiElems.Content, Position = UDim2.new(0, 150, 0, 4), Size = UDim2.new(0, 110, 0, 24), Text = "Save to File", TextColor3 = Color3.new(1, 1, 1), TextSize = 12, } },
                    { 2, "UICorner",   { CornerRadius = UDim.new(0, 6), Parent = { 1 }, } },
                })
                Lib.ButtonAnim(save,
                    { Mode = 2, StartColor = Color3.fromRGB(45, 45, 45), LerpTo = Color3.fromRGB(0, 162, 255), LerpDelta = 0.15 })
                ...
                local dumpbtn = create({
                    { 1, "TextButton", { BackgroundColor3 = Color3.fromRGB(45, 45, 45), BorderSizePixel = 0, Font = Enum.Font.Gotham, Name = "Dump", Parent = window.GuiElems.Content, Position = UDim2.new(0, 265, 0, 4), Size = UDim2.new(0, 130, 0, 24), Text = "Dump Functions", TextColor3 = Color3.new(1, 1, 1), TextSize = 12, } },
                    { 2, "UICorner",   { CornerRadius = UDim.new(0, 6), Parent = { 1 }, } },
                })
                Lib.ButtonAnim(dumpbtn,
                    { Mode = 2, StartColor = Color3.fromRGB(45, 45, 45), LerpTo = Color3.fromRGB(0, 162, 255), LerpDelta = 0.15 })
                            ...
                            function functions:dump_table(input, indent, index)
                                ...
                                for index, value in pairs(input) do
                                    ...
                                    if type(value) == "function" then
                                        functions:add_to_dump(
                                        ("%d [function] = %s"):format(count, functions:get_function_name(value)), indent)
                                    elseif type(value) == "table" then
                                        ...
                                        functions:add_to_dump(
                                        ("%d [%s] = %s"):format(count, tostring(typeof(value)), tostring(value)), indent)
                                    end
                                end
                            end

                            function functions:dump_function(input, indent)
                                functions:add_to_dump(("\nFunction Dump: %s"):format(functions:get_function_name(input)),
                                    indent)
                                functions:add_to_dump(
    ...
    Gemini chat + Dex tool access
        ...
        local function main()
            local AI = {}
            local window
            local chatScroll, inputBox, sendButton, statusLabel, messageLayout
            local history = {}
            local chatMessages = {}
            local CHAT_FILE = "dex/ai_chat.json"
            local isGenerating = false
            local toolDepth = 0
            local MAX_TOOL_CALLS = 6
            local DEFAULT_LIST_LIMIT = 200
            local MAX_LIST_LIMIT = 2000
            local DEFAULT_DECOMPILE_MAXCHARS = 20000
            local MAX_DECOMPILE_MAXCHARS = 200000
            local DEFAULT_CONTEXT_MESSAGES = 60

            local root = game["Run Service"].Parent
            local getDescendants = root.GetDescendants
            local getChildren = root.GetChildren
            local isA = root.IsA

            local function clampNumber(value, minValue, maxValue, defaultValue)
                local num = tonumber(value)
                if not num then return defaultValue end
                if num < minValue then return minValue end
                if num > maxValue then return maxValue end
                return num
            end

            local function safeTrim(text, maxLen)
                if type(text) ~= "string" then return "" end
                if not maxLen or #text <= maxLen then return text end
            ...
            local function renderInline(text)
                ...
                text = text:gsub("(\n?)###%s*(.-)(\n?)", "%1<font size=\"18\"><b>%2</b></font>%3")
                text = text:gsub("(\n?)##%s*(.-)(\n?)", "%1<font size=\"20\"><b>%2</b></font>%3")
                text = text:gsub("(\n?)#%s*(.-)(\n?)", "%1<font size=\"22\"><b>%2</b></font>%3")
            ...
            local function markdownToRichText(text)
                ...
                local pos = 1
                while true do
                    local s, e, lang, code = text:find("```([^\n]*)\n(.-)\n```", pos)
                    if not s then
                        out[#out + 1] = renderInline(text:sub(pos))
                        break
                    end
                    if s > pos then
                        out[#out + 1] = renderInline(text:sub(pos, s - 1))
                    end
                    local escaped = escapeText(code)
                    local header = ""
                    if lang and lang ~= "" then
                        header = "<font size=\"12\" color=\"#9EA4B0\"><b>" .. escapeText(lang) .. "</b></font>\n"
                    end
                    out[#out + 1] = header .. "<font face=\"Code\" size=\"12\">" .. escaped .. "</font>"
                    pos = e + 1
                end
            ...
            local function updateCanvas()
                if not chatScroll or not messageLayout then return end
                chatScroll.CanvasSize = UDim2.new(0, 0, 0, messageLayout.AbsoluteContentSize.Y + 8)
                chatScroll.CanvasPosition = Vector2.new(0, chatScroll.CanvasSize.Y.Offset)
            end

            local function isPersistChat()
                return Main and Main.AISettings and Main.AISettings.PersistChat == true
            end

            local function saveChat()
                if not isPersistChat() then return end
                if not env or not env.writefile then return end
                local payload = { Version = 1, Messages = chatMessages }
                pcall(env.writefile, CHAT_FILE, service.HttpService:JSONEncode(payload))
            end

            local function loadChat()
                if not isPersistChat() then return nil end
                if not env or not env.readfile then return nil end
                local ok, raw = pcall(env.readfile, CHAT_FILE)
                if not ok or not raw or raw == "" then return nil end
                local okDecode, decoded = pcall(service.HttpService.JSONDecode, service.HttpService, raw)
                if not okDecode or type(decoded) ~= "table" then return nil end
                if type(decoded.Messages) ~= "table" then return nil end
                local out = {}
                for i = 1, #decoded.Messages do
                    local m = decoded.Messages[i]
            ...
            local function recordChat(role, text)
                if not isPersistChat() then return end
                chatMessages[#chatMessages + 1] = { Role = role, Text = safeTrim(text, 80000) }
                if #chatMessages > 120 then
                    table.remove(chatMessages, 1)
                end
                saveChat()
            end

            local function addMessage(role, text, persist)
                if not chatScroll then return end

                local availableWidth = chatScroll.AbsoluteSize.X
                if availableWidth <= 0 then
                    availableWidth = window and window.GuiElems.Content.AbsoluteSize.X or 520
                end
                local maxWidth = math.max(200, availableWidth - 24)
                local plainText = text:gsub("```", ""):gsub("[*_`#]", "")
                local textBounds = service.TextService:GetTextSize(plainText, 13, Enum.Font.Gotham,
                    Vector2.new(maxWidth, 10000))
                local bubbleWidth = math.min(maxWidth, math.max(120, textBounds.X + 12))
                local bubbleHeight = textBounds.Y + 12

                local row = createSimple("Frame", {
                    Parent = chatScroll,
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    Size = UDim2.new(1, 0, 0, bubbleHeight)
                })

                local bubble = createSimple("Frame", {
                    Parent = row,
                    BackgroundColor3 = (role == "User" and Settings.Theme.Button) or
                    (role == "Dex" and Settings.Theme.Outline2) or Settings.Theme.Main1,
                    BorderSizePixel = 0,
                ...
                if persist ~= false then
                    recordChat(role, text)
                end
            end

            local toolGuide = table.concat({
                "You are Dex AI (Gemini inside DarkDex).",
                "Dil: Kullanici Turkce yaziyorsa Turkce yanitla (aksi halde kullanicinin dilini takip et).",
                "",
                "Canli oyun verisi gerektiginde TEK SATIRLIK arac cagrisi yapabilirsin.",
                "COK ONEMLI: Araci kullandiginda SADECE 1 satir yaz ve baska hicbir sey ekleme:",
                "@DEX {\"action\":\"children\",\"path\":\"game:GetService(\\\"Players\\\")\",\"limit\":200}",
                "",
                "Arac cagrilarini code block icine alma. JSON'un dis yapisindaki tirnaklari (\"action\" gibi) ASLA kacirma.",
                "Not: String degerlerinin icinde gerekiyorsa kacis kullanabilirsin (ornegin path icinde game:GetService(\\\"Players\\\")).",
                "Yanlis ornek (YAPMA): @DEX {\\\"action\\\":\\\"children\\\"}",
                "Dogru ornek: @DEX {\"action\":\"children\",\"path\":\"game:GetService(\\\"Players\\\")\",\"limit\":200}",
                "Arac sonucu geldikten sonra normal yanit ver. Gerekirse ikinci bir @DEX satiri daha cagirabilirsin.",
                "",
                "Hedef belirleme (Instance secme):",
                "- Tercih: {\"path\":\"...\"} (Dex path) ornek: game:GetService(\"Players\").LocalPlayer.PlayerGui",
                "- Veya: {\"selectionIndex\":1} (Explorer secimi)",
                "- Veya: {\"fullName\":\"Workspace.Model.Part\"} (GetFullName)",
                "- Veya: {\"name\":\"PartName\",\"class\":\"Part\",\"index\":1,\"exact\":false}",
                "",
                "Eylemler (hepsi {\"action\":\"...\"} kullanir):",
                "1) search: Isme gore bul (substring) + opsiyonel class. Args: query/name, class, limit, within(optional target/path).",
                "2) children: Hedefin direk cocuklarini listele. Args: target, limit.",
                "3) selection: Mevcut Explorer secimini listele.",
                "4) select: Explorer'da hedefi sec ve gorunur yap. Args: target OR name/class/index/exact.",
                "5) info: Hedef ozeti (Name, Class, Parent, Path). Args: target.",
                "6) properties: Okunabilen property'leri oku. Args: target, props(optional array), limit.",
                "7) decompile: Script/ModuleScript/LocalScript decompile. Args: target, maxChars.",
                "8) remotes: RemoteEvent/RemoteFunction listele. Args: limit.",
                "9) path: Hedefin Dex path string'ini ver. Args: target.",
                "",
                "Kurallar:",
                "- Arac sonucunu gormeden uydurma yapma.",
                "- 'Target not found' / 'No target instance' alirsan search + select ile yolu duzelt.",
                "- Uzun listelerde limit'i yuksek tutabilirsin ama okunabilirligi koru."
            }, "\n")

            local function resetHistory()
                history = {
                    { role = "user", parts = { { text = toolGuide } } }
                }
            end

            local function formatInstance(obj)
                if not obj then return nil end
                local fullName
                pcall(function() fullName = obj:GetFullName() end)
                local path
                pcall(function() path = Explorer.GetInstancePath(obj) end)
                return {
                    Name = obj.Name,
            ...
            local function formatInstanceList(list, limit)
                if not list or #list == 0 then return "No results." end
                local lines = {}
                local maxCount = limit or #list
                for i = 1, math.min(#list, maxCount) do
                    local item = list[i]
                    local path = item.Path
                    if not path or path == "" then
                        path = item.FullName or ""
                    end
                    lines[#lines + 1] = ("%d) %s (%s)\n%s"):format(i, item.Name, item.ClassName, path)
                end
                if #list > maxCount then
                    lines[#lines + 1] = ("...and %d more"):format(#list - maxCount)
                end
                return table.concat(lines, "\n")
            end

            local function resolvePathExpr(pathExpr)
                if type(pathExpr) ~= "string" then return nil end
                local s = pathExpr:gsub("^%s+", ""):gsub("%s+$", "")
                if s == "" then return nil end

                local svcName = s:match("^game:GetService%(%s*\"([^\"]+)\"%s*%)") or
                    s:match("^game:GetService%(%s*'([^']+)'%s*%)")
                local obj
                local rest = ""
                if svcName then
                    local ok, svc = pcall(game.GetService, game, svcName)
                    if ok then obj = svc end
                    rest = s:match("^game:GetService%(%s*['\"][^'\"]+['\"]%s*%)(.*)$") or ""
                elseif s:sub(1, 5) == "game." then
                    obj = game
                    rest = s:sub(5)
                elseif s == "game" then
                    obj = game
                ...
                while r ~= "" do
                    if r:sub(1, 1) == "." then
                        r = r:sub(2)
                        local name = r:match("^([%w_]+)")
                        if not name or name == "" then break end
                        r = r:sub(#name + 1)
                        obj = obj:FindFirstChild(name)
                        if not obj then return nil end
                    elseif r:sub(1, 1) == "[" then
                        local name = r:match("^%[\"(.-)\"%]") or r:match("^%['(.-)'%]")
                        if not name then break end
                        local closePos = r:find("%]", 1, true)
                        r = closePos and r:sub(closePos + 1) or ""
                        obj = obj:FindFirstChild(name)
                        if not obj then return nil end
                    else
                        break
            ...
            local function findByPath(path)
                ...
                local direct = resolvePathExpr(path)
                if direct then return direct end
                local list = getDescendants(root)
                for i = 1, #list do
                    local obj = list[i]
                    local ok, res = pcall(Explorer.GetInstancePath, obj)
                    if ok and res == path then
                        return obj
                    end
                end
                if env.getnilinstances then
                    local nilList = env.getnilinstances()
                    for i = 1, #nilList do
                        local obj = nilList[i]
                        local ok, res = pcall(Explorer.GetInstancePath, obj)
                        if ok and res == path then
            ...
            local function findByName(name, className, index, exact)
                if not name or name == "" then return nil end
                local lower = string.lower
                local matches = {}
                local query = lower(name)
                local list = getDescendants(root)
                local classLower = className and lower(className) or nil

                for i = 1, #list do
                    local obj = list[i]
                    local objName = lower(obj.Name)
                    if (exact and objName == query) or (not exact and string.find(objName, query, 1, true)) then
                        local classMatch = true
                        if classLower then
                            classMatch = lower(obj.ClassName) == classLower
                            if not classMatch then
                                local ok = pcall(isA, obj, className)
                                classMatch = ok and isA(obj, className)
                            end
                        end
                        if classMatch then
                ...
                local pick = index or 1
                return matches[pick], matches
            end

            local function getSelectionList()
                if not Explorer or not Explorer.Selection then return {} end
                local sList = Explorer.Selection.List or {}
                local results = {}
                for i = 1, #sList do
                    local obj = sList[i].Obj
                    local item = formatInstance(obj)
            ...
            local function resolveTarget(args)
                args = args or {}
                local obj
                if args.selectionIndex and Explorer and Explorer.Selection then
                    local node = Explorer.Selection.List[args.selectionIndex]
                    obj = node and node.Obj
                end
                if not obj and args.path then
                    obj = findByPath(args.path)
                end
                if not obj and args.fullName then
                    local list = getDescendants(root)
                    for i = 1, #list do
                        if list[i]:GetFullName() == args.fullName then
                            obj = list[i]
                            break
                        end
                    end
                end
                if not obj and args.name then
                    obj = select(1, findByName(args.name, args.class, args.index, args.exact))
                end
                if not obj and Explorer and Explorer.Selection then
            ...
            local function formatPropValue(prop, val)
                if val == nil then return "nil" end
                local valType = typeof(val)
                if valType == "Instance" then
                    local ok, fullName = pcall(function() return val:GetFullName() end)
                    return ok and fullName or tostring(val)
                end
                if Properties and prop then
                    local ok, str = pcall(Properties.ValueToString, prop, val)
                    if ok and str then return str end
                end
                return tostring(val)
            end

            local function getPropertiesText(obj, props, limit)
                if not obj then return "No target instance." end
                if not API or not API.Classes or not Properties then return "Properties not available." end
                local classData = API.Classes[obj.ClassName]
                if not classData then return "Class not found in API: " .. tostring(obj.ClassName) end
                ...
                local propList = Properties.GetIndexableProps(obj, classData) or {}
                local lines = {}
                local count = 0
                for i = 1, #propList do
                    local prop = propList[i]
                    if next(wanted) == nil or wanted[prop.Name] then
                        local ok, val = pcall(Properties.GetPropVal, prop, obj)
                        if ok then
                            lines[#lines + 1] = prop.Name .. " = " .. formatPropValue(prop, val)
                            count = count + 1
                            if limit and count >= limit then break end
                        end
                    end
                end
                if #lines == 0 then return "No readable properties." end
                return table.concat(lines, "\n")
            end

            local function listChildren(obj, limit)
                if not obj then return "No target instance." end
                local ch = getChildren(obj)
                local results = {}
                for i = 1, #ch do
                    local item = formatInstance(ch[i])
                    if item then results[#results + 1] = item end
                    if limit and #results >= limit then break end
                end
                return formatInstanceList(results, limit)
            end

            local function listRemotes(limit)
                local results = {}
                local list = getDescendants(root)
                for i = 1, #list do
                    local obj = list[i]
                    if isA(obj, "RemoteEvent") or isA(obj, "RemoteFunction") then
                        local item = formatInstance(obj)
                        if item then results[#results + 1] = item end
                        if limit and #results >= limit then break end
                    end
                end
                if env.getnilinstances and (not limit or #results < limit) then
                    local nilList = env.getnilinstances()
                    for i = 1, #nilList do
                        local obj = nilList[i]
                        if isA(obj, "RemoteEvent") or isA(obj, "RemoteFunction") then
            ...
            local function decompileScript(obj, maxChars)
                if not obj then return "No target instance." end
                if not isA(obj, "LuaSourceContainer") then
                    return "Target is not a script."
                end
                if env.decompile then
                    local ok, source = pcall(env.decompile, obj)
                    if ok and source and source ~= "" then
                        if maxChars and #source > maxChars then
                ...
                return "Source failed to decompile."
            end

            local function runTool(data)
                if type(data) ~= "table" then return "Invalid tool data." end
                local action = data.action or data.tool
                if not action then return "Missing action." end
                action = string.lower(tostring(action))

                if action == "search" then
                    local query = data.query or data.name
                    local className = data.class
                    local limit = clampNumber(data.limit, 1, MAX_LIST_LIMIT, DEFAULT_LIST_LIMIT)
                    local list
                    local scopeObj
                    if data.within ~= nil then
                        if type(data.within) == "table" then
                            scopeObj = resolveTarget(data.within)
                        else
                            scopeObj = findByPath(tostring(data.within))
                        end
                    elseif data.withinPath ~= nil then
                        scopeObj = findByPath(tostring(data.withinPath))
                    end
                    if scopeObj then
                        local ok, scoped = pcall(function() return scopeObj:GetDescendants() end)
                        list = (ok and scoped) or getDescendants(root)
                    else
                        list = getDescendants(root)
                    end
                    local results = {}
                    local lower = string.lower
                    local q = query and lower(query) or nil
                    local classLower = className and lower(className) or nil
                    for i = 1, #list do
                        local obj = list[i]
                        if not q or string.find(lower(obj.Name), q, 1, true) then
                            local classMatch = true
                            if classLower then
                                classMatch = lower(obj.ClassName) == classLower
                                if not classMatch then
                ...
                elseif action == "selection" then
                    return formatInstanceList(getSelectionList())
                elseif action == "select" then
                    local obj, matches = nil, nil
                    if data.name then
                        obj, matches = findByName(data.name, data.class, data.index, data.exact)
                    else
                        obj = resolveTarget(data)
                    end
                    if not obj then return "Target not found." end
                    local node = nodes[obj]
                    if not node then return "Target not in explorer tree." end
                ...
                elseif action == "info" then
                    local obj = resolveTarget(data)
                    if not obj then return "Target not found." end
                    local item = formatInstance(obj)
                    if not item then return "Target not readable." end
                    local parentName = obj.Parent and obj.Parent.Name or "nil"
                    return ("Name: %s\nClass: %s\nParent: %s\nPath: %s"):format(item.Name, item.ClassName, parentName,
                        item.Path)
                elseif action == "properties" then
                    local obj = resolveTarget(data)
                    return getPropertiesText(obj, data.props, clampNumber(data.limit, 1, MAX_LIST_LIMIT, DEFAULT_LIST_LIMIT))
                elseif action == "decompile" then
                    local obj = resolveTarget(data)
                    return decompileScript(obj,
                        clampNumber(data.maxChars, 1000, MAX_DECOMPILE_MAXCHARS, DEFAULT_DECOMPILE_MAXCHARS))
                elseif action == "remotes" then
                    return listRemotes(clampNumber(data.limit, 1, MAX_LIST_LIMIT, DEFAULT_LIST_LIMIT))
                ...
                elseif action == "children" then
                    local obj = resolveTarget(data)
                    return listChildren(obj, clampNumber(data.limit, 1, MAX_LIST_LIMIT, DEFAULT_LIST_LIMIT))
                else
            ...
            local function parseToolCall(text)
                ...
                local jsonText = text:match("@DEX%s*(%b{})")
                if not jsonText then return nil end

                local function tryDecode(str)
                    local ok, data = pcall(service.HttpService.JSONDecode, service.HttpService, str)
                    if ok and type(data) == "table" then return data end
                    return nil
                end

                local decoded = tryDecode(jsonText)
                if decoded then return decoded end

                -- Common model mistake: escaping the JSON structural quotes like {\"action\":\"...\"}
                local cleaned = jsonText:gsub("\\\"", "\"")
                decoded = tryDecode(cleaned)
                if decoded then return decoded end
                return nil
            end

            local function extractCandidateText(decoded)
                if not decoded then return nil end

                local function functionCallToDexLine(fc)
                    if type(fc) ~= "table" then return nil end
                    local args = fc.args or fc.arguments or fc.params
                    if type(args) == "string" then
                        local ok, decodedArgs = pcall(service.HttpService.JSONDecode, service.HttpService, args)
                        if ok then args = decodedArgs end
                    end
                    local payload = type(args) == "table" and args or {}
                    if payload.action == nil and payload.tool == nil and type(fc.name) == "string" then
                        payload.action = fc.name
                    end
                    local ok, json = pcall(service.HttpService.JSONEncode, service.HttpService, payload)
                    if not ok or type(json) ~= "string" then return nil end
                    return "@DEX " .. json
                end

                local candidates = decoded.candidates
                if type(candidates) ~= "table" or not candidates[1] then return nil end

                for c = 1, #candidates do
                    local cand = candidates[c]
                    if type(cand) == "table" then
                        if type(cand.output) == "string" and cand.output ~= "" then
                            return cand.output
                        end
                        if type(cand.text) == "string" and cand.text ~= "" then
                            return cand.text
                        end
                        if cand.functionCall then
                            local line = functionCallToDexLine(cand.functionCall)
                        ...
                        if type(content) == "table" and type(content.text) == "string" and content.text ~= "" then
                            return content.text
                        end
                        local parts = type(content) == "table" and content.parts or nil
                        if type(parts) == "table" then
                            if type(parts.text) == "string" and parts.text ~= "" then
                                return parts.text
                            end
                            local out = {}
                            for i = 1, #parts do
                                ...
                                if type(part) == "table" then
                                    ...
                                    elseif part.functionCall then
                                        local line = functionCallToDexLine(part.functionCall)
            ...
            local function parseJsonResponse(raw)
                if type(raw) == "table" then return raw end
                if type(raw) ~= "string" then return nil end
                raw = raw:gsub("^[%z\1-\31]+", ""):gsub("[%z\1-\31]+$", "")
                local ok, decoded = pcall(service.HttpService.JSONDecode, service.HttpService, raw)
                if ok then return decoded end
                local startPos = raw:find("{", 1, true)
                local endPos
                for i = #raw, 1, -1 do
                    if raw:sub(i, i) == "}" then
                        endPos = i
                ...
                if startPos and endPos and endPos > startPos then
                    local ok2, decoded2 = pcall(service.HttpService.JSONDecode, service.HttpService,
                        raw:sub(startPos, endPos))
                    if ok2 then return decoded2 end
                end
                return nil
            end

            local function requestGemini(opts)
                opts = opts or {}
                local apiKey = Main.AISettings and Main.AISettings.ApiKey or ""
                if apiKey == "" then
                    return nil, "Missing Gemini API key. Open Settings and add it."
                end
                local model = Main.AISettings and Main.AISettings.Model or "gemini-3-flash-preview"
                local url = ("https://generativelanguage.googleapis.com/v1beta/models/%s:generateContent?key=%s")
                    :format(model, apiKey)

                local maxContext = DEFAULT_CONTEXT_MESSAGES
                if Main.AISettings and Main.AISettings.MaxContextMessages then
                    maxContext = clampNumber(Main.AISettings.MaxContextMessages, 6, 200, DEFAULT_CONTEXT_MESSAGES)
                end

                local baseHistory = opts.History or history
                local reqHistory = baseHistory
                if #baseHistory > (maxContext + 1) then
                    local startIndex = #baseHistory - maxContext + 1
                    local trimmed = { baseHistory[1] }
                    for i = startIndex, #baseHistory do
                        trimmed[#trimmed + 1] = baseHistory[i]
                    end
                    reqHistory = trimmed
                end

                local maxOut = 8192
                if Main.AISettings and Main.AISettings.MaxOutputTokens then
                    maxOut = clampNumber(Main.AISettings.MaxOutputTokens, 256, 32768, 8192)
                end
                ...
                if env.request then
                    local ok, res = pcall(env.request, {
                        ...
                        Body = service.HttpService:JSONEncode(body)
                    })
                    if ok and res then
                        response = res.Body or res.body or res.ResponseBody or res.Response
                        statusCode = res.StatusCode or res.Status or res.status
                        if not response and type(res) == "string" then
                            response = res
                        end
                    end
                else
                    local ok, res = pcall(function()
                        return service.HttpService:PostAsync(url, service.HttpService:JSONEncode(body),
                            Enum.HttpContentType.ApplicationJson)
                    end)
                    if ok and res then
                        response = res
                    else
                        return nil, "HttpService request failed."
                    end
                end
                ...
                local decoded = parseJsonResponse(response)
                if not decoded then
                    local preview = tostring(response)
                    preview = preview:gsub("[%z\1-\31]", "")
                    if #preview > 240 then preview = preview:sub(1, 240) .. "..." end
                    return nil, "Gemini response decode failed.\nRaw preview: " .. preview
                end

                local text = extractCandidateText(decoded)
                if text then
                    return text
                end

                local cand = decoded and decoded.candidates and decoded.candidates[1] or nil
                if decoded and decoded.error and decoded.error.message then
                    return nil, "Gemini error: " .. tostring(decoded.error.message)
                end
                if decoded and decoded.promptFeedback and decoded.promptFeedback.blockReason then
                    return nil, "Gemini blocked: " .. tostring(decoded.promptFeedback.blockReason)
                end
                if statusCode and tonumber(statusCode) and tonumber(statusCode) ~= 200 then
                    return nil, "Gemini HTTP " .. tostring(statusCode)
                end

                if not opts._didMinimalRetry then
                    local lastUserText
                    for i = #baseHistory, 1, -1 do
                        local msg = baseHistory[i]
                        if type(msg) == "table" and msg.role == "user" and msg.parts and msg.parts[1] and
                            type(msg.parts[1].text) == "string" then
                            lastUserText = msg.parts[1].text
                            break
                        end
                    end
                    if lastUserText and lastUserText ~= "" then
                        local minimalGuide = table.concat({
                            "You are a helpful assistant.",
                            "If you need live game data, reply with exactly one line starting with @DEX and a JSON object.",
                            "Otherwise answer normally."
                        }, "\n")
                        local minimalHistory = {
                            { role = "user", parts = { { text = minimalGuide } } },
                            { role = "user", parts = { { text = lastUserText } } }
                        }
                        return requestGemini({ History = minimalHistory, _didMinimalRetry = true })
                    end
                end

                if cand and cand.finishReason then
                    return nil, "Gemini finishReason: " .. tostring(cand.finishReason)
                end

                local function keysPreview(t, maxCount)
                    if type(t) ~= "table" then return "" end
                    local keys = {}
                    for k in pairs(t) do
                        keys[#keys + 1] = tostring(k)
                        if maxCount and #keys >= maxCount then break end
                    end
                    table.sort(keys)
                    return table.concat(keys, ", ")
                end

                local topKeys = keysPreview(decoded, 12)
                local candKeys = keysPreview(cand, 12)
                local candPreview = ""
                if cand then
                    local ok, json = pcall(service.HttpService.JSONEncode, service.HttpService, cand)
                    if ok and type(json) == "string" then
                        json = json:gsub("[%z\1-\31]", "")
                        if #json > 240 then json = json:sub(1, 240) .. "..." end
                        candPreview = json
                    end
                end
                return nil, ("Gemini response unreadable.\nKeys: %s\nCandidate keys: %s%s"):format(
                    topKeys ~= "" and topKeys or "none",
                    candKeys ~= "" and candKeys or "none",
                    candPreview ~= "" and ("\nCandidate preview: " .. candPreview) or "")
            end
            ...
            local function sendMessage()
                ...
                local userTool = parseToolCall(text)
                if userTool then
                    table.insert(history, { role = "user", parts = { { text = text } } })
                    local result = runTool(userTool)
                    addMessage("Dex", result)
                    table.insert(history, { role = "user", parts = { { text = "Dex tool result:\n" .. result } } })
                    return
                end
                ...
                task.spawn(function()
                    local response, err = requestGemini()
                    if not response then
                        statusLabel.Text = ""
                        isGenerating = false
                        addMessage("Gemini", err or "Request failed.")
                        return
                    end

                    local loops = 0
                    while loops < MAX_TOOL_CALLS do
                        local toolData = parseToolCall(response)
                        if not toolData then
                            addMessage("Gemini", response)
                            table.insert(history, { role = "model", parts = { { text = response } } })
                            break
                        end

                        loops = loops + 1
                        table.insert(history, { role = "model", parts = { { text = response } } })
                        local result = runTool(toolData)
                        addMessage("Dex", result)
                        table.insert(history, { role = "user", parts = { { text = "Dex tool result:\n" .. result } } })

                        local follow, err2 = requestGemini()
                        if not follow then
                            addMessage("Gemini", err2 or "Tool result sent.")
                            break
                        end
                        response = follow
                    end
            ...
            AI.Init = function()
                ...
                local chatFrame = createSimple("Frame", {
                    Parent = window.GuiElems.Content,
                    BackgroundTransparency = 1,
                    Position = UDim2.new(0, 6, 0, 6),
                    Size = UDim2.new(1, -12, 1, -48)
                })
                chatScroll = createSimple("ScrollingFrame", {
                    Parent = chatFrame,
                    Active = true,
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                ...
                messageLayout = createSimple("UIListLayout", {
                    Parent = chatScroll,
                    Padding = UDim.new(0, 6),
                    SortOrder = Enum.SortOrder.LayoutOrder
                })
                messageLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateCanvas)

                local inputFrame = createSimple("Frame", {
                    Parent = window.GuiElems.Content,
                ...
                createSimple("UICorner", { CornerRadius = UDim.new(0, 6), Parent = inputFrame })
                ...
                Lib.ViewportTextBox.convert(inputBox)
                ...
                createSimple("UICorner", { CornerRadius = UDim.new(0, 6), Parent = sendButton })
                Lib.ButtonAnim(sendButton,
                    { Mode = 2, StartColor = Settings.Theme.Button, LerpTo = Settings.Theme.ButtonHover, LerpDelta = 0.2 })
                ...
                if loaded and #loaded > 0 then
                    ...
                    for i = 1, #loaded do
                        ...
                        if m.Role == "User" then
                            table.insert(history, { role = "user", parts = { { text = m.Text } } })
                        elseif m.Role == "Gemini" then
                            table.insert(history, { role = "model", parts = { { text = m.Text } } })
                        elseif m.Role == "Dex" then
                            table.insert(history, { role = "user", parts = { { text = "Dex tool result:\n" .. m.Text } } })
                        end
                    end
                else
                    addMessage("Gemini", "Hello! I am Gemini AI.")
                    table.insert(history, { role = "model", parts = { { text = "Hello! I am Gemini AI." } } })
                end
            end
	...
	Container for functions and classes
        ...
        local function main()
            ...
            local function getGuiMT(props, funcs)
                return {
                    __index = function(self, ind) if not props[ind] then return funcs[ind] or self.Gui[ind] end end,
                    __newindex = function(self, ind, val) if not props[ind] then self.Gui[ind] = val else rawset(self,
                                ind, val) end end
                }
            end
            ...
            Lib.ScrollBar = (function()
                ...
                local function createFrame(self)
                    local newFrame = createSimple("Frame",
                        { Style = 0, Active = true, AnchorPoint = Vector2.new(0, 0), BackgroundColor3 = Color3.fromRGB(
                        25, 25, 25), BackgroundTransparency = 1, BorderSizePixel = 0, ClipsDescendants = false, Draggable = false, Position =
                        UDim2.new(1, -10, 0, 0), Rotation = 0, Selectable = false, Size = UDim2.new(0, 8, 1, 0), SizeConstraint = 0, Visible = true, ZIndex = 1, Name =
                        "ScrollBar", })
                    local button1 = nil
                    local button2 = nil

                    if self.Horizontal then
                        newFrame.Size = UDim2.new(1, 0, 0, 8)
                        ...
                        newFrame.Size = UDim2.new(0, 8, 1, 0)
                        button1 = createSimple("ImageButton", {
                            Parent = newFrame,
                            Name = "Up",
                            Size = UDim2.new(0, 0, 0, 0),
                    ...
                    if self.Horizontal then
                        scrollThumbFrame.Position = UDim2.new(0, 0, 0, 0)
                        scrollThumbFrame.Size = UDim2.new(1, 0, 1, 0)
                    else
                        scrollThumbFrame.Position = UDim2.new(0, 0, 0, 0)
                        scrollThumbFrame.Size = UDim2.new(1, 0, 1, 0)
                    end
                    ...
                    createSimple("UICorner", { CornerRadius = UDim.new(1, 0), Parent = scrollThumb })
                    ...
                    scrollThumb.InputBegan:Connect(function(input)
                        ...
                        releaseEvent = user.InputEnded:Connect(function(input)
                            ...
                            if checkMouseInGui(scrollThumb) then scrollThumb.BackgroundTransparency = 0.2 else
                                scrollThumb.BackgroundTransparency = 0
                        ...
                        mouseEvent = user.InputChanged:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.MouseMovement and thumbPress and releaseEvent.Connected then
                                local thumbFrameSize = scrollThumbFrame.AbsoluteSize[dir] - scrollThumb.AbsoluteSize
                                [dir]
                ...
                funcs.SetZIndex = function(self, base)
                    local elems = self.GuiElems
                    local childZ = base + 1
                    self.Gui.ZIndex = base
                    if elems.ScrollThumbFrame then elems.ScrollThumbFrame.ZIndex = childZ end
                    if elems.ScrollThumb then elems.ScrollThumb.ZIndex = childZ + 1 end
                    if elems.MarkerFrame then elems.MarkerFrame.ZIndex = childZ + 1 end
                    if elems.Button1 then elems.Button1.ZIndex = childZ + 1 end
                    if elems.Button2 then elems.Button2.ZIndex = childZ + 1 end
                end
                ...
                funcs.SetScrollFrame = function(self, frame)
                    ...
                    self.ScrollUpEvent = frame.MouseWheelForward:Connect(function() self:ScrollTo(self.Index -
                        self.WheelIncrement) end)
                    self.ScrollDownEvent = frame.MouseWheelBackward:Connect(function() self:ScrollTo(self.Index +
                        self.WheelIncrement) end)
                end
                ...
                local function new(hor)
                    ...
                    obj:Texture({
                        ...
                        ThumbSelectColor = Color3.fromRGB(100, 100, 100),
            ...
            Lib.Window = (function()
                ...
                local theme = {
                    MainColor1 = Color3.fromRGB(25, 25, 25),
                    MainColor2 = Color3.fromRGB(35, 35, 35),
                ...
                local function resizeHook(self, resizer, dir)
                    ...
                    resizer.InputBegan:Connect(function(input)
                        if not self.Dragging and not self.Resizing and self.Resizable and self.ResizableInternal then
                            ...
                            local bar = resizer:FindFirstChild("Bar")
                            local bar2 = resizer:FindFirstChild("Bar2")
                            local targetElement = bar or resizer

                            if input.UserInputType == Enum.UserInputType.MouseMovement then
                                service.TweenService:Create(targetElement, TweenInfo.new(0.1), { BackgroundTransparency = 0.4 }):Play()
                                if bar2 then service.TweenService:Create(bar2, TweenInfo.new(0.1), { BackgroundTransparency = 0.4 }):Play() end
                            elseif input.UserInputType == Enum.UserInputType.MouseButton1 then
                                local releaseEvent, mouseEvent
                                ...
                                service.TweenService:Create(targetElement, TweenInfo.new(0.1), { BackgroundTransparency = 0 }):Play()
                                if bar2 then service.TweenService:Create(bar2, TweenInfo.new(0.1), { BackgroundTransparency = 0 }):Play() end
                                ...
                                releaseEvent = service.UserInputService.InputEnded:Connect(function(input)
                                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                                        ...
                                        mouseEvent:Disconnect()
                                        self.Resizing = false
                                        service.TweenService:Create(targetElement, TweenInfo.new(0.1), { BackgroundTransparency = 0.8 }):Play()
                                        if bar2 then service.TweenService:Create(bar2, TweenInfo.new(0.1), { BackgroundTransparency = 0.8 }):Play() end
                                    end
                                end)
                                ...
                                mouseEvent = service.UserInputService.InputChanged:Connect(function(input)
                                    if self.Resizable and self.ResizableInternal and input.UserInputType == Enum.UserInputType.MouseMovement then
                                        ...
                                        if guiMain.AbsoluteSize.X + deltaX * signX < self.MinX then deltaX = signX *
                                            (self.MinX - guiMain.AbsoluteSize.X) end
                                        if guiMain.AbsoluteSize.Y + deltaY * signY < self.MinY then deltaY = signY *
                                            (self.MinY - guiMain.AbsoluteSize.Y) end
                                        if signY < 0 and guiMain.AbsolutePosition.Y + deltaY < 0 then deltaY = -guiMain
                                            .AbsolutePosition.Y end
                ...
                local createGui = function(self)
                    local gui = create({
                        ...
                        { 2,  "Frame",      { Active = true, BackgroundColor3 = theme.MainColor1, BorderSizePixel = 0, Name = "Main", Parent = { 1 }, Position = UDim2.new(0.5, -150, 0.5, -150), Size = UDim2.new(0, 300, 0, 300), } },
                        { 3,  "UICorner",   { CornerRadius = UDim.new(0, 8), Parent = { 2 }, } },
                        { 4,  "UIStroke",   { Color = Color3.fromRGB(45, 45, 45), Thickness = 1, Transparency = 0.5, Parent = { 2 }, } },
                        { 5,  "Frame",      { BackgroundColor3 = theme.MainColor2, BorderSizePixel = 0, Name = "Content", Parent = { 2 }, Position = UDim2.new(0, 0, 0, 30), Size = UDim2.new(1, 0, 1, -30), ClipsDescendants = true } },
                        { 6,  "UICorner",   { CornerRadius = UDim.new(0, 8), Parent = { 5 }, } },
                        { 7,  "Frame",      { BackgroundColor3 = Color3.fromRGB(40, 40, 40), BorderSizePixel = 0, Name = "Line", Parent = { 5 }, Size = UDim2.new(1, 0, 0, 1), } },
                        { 8,  "Frame",      { BackgroundColor3 = theme.MainColor1, BorderSizePixel = 0, Name = "TopBar", Parent = { 2 }, Size = UDim2.new(1, 0, 0, 30), } },
                        { 9,  "UICorner",   { CornerRadius = UDim.new(0, 8), Parent = { 8 }, } },
                        { 10, "TextLabel",  { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.GothamMedium, Name = "Title", Parent = { 8 }, Position = UDim2.new(0, 10, 0, 0), Size = UDim2.new(1, -20, 0, 30), Text = "Window", TextColor3 = theme.Text, TextSize = 14, TextXAlignment = 0, } },
                        { 11, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.fromRGB(200, 50, 50), BackgroundTransparency = 0.2, BorderSizePixel = 0, Font = 3, Name = "Close", Parent = { 8 }, Position = UDim2.new(1, -26, 0, 7), Size = UDim2.new(0, 16, 0, 16), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, } },
                        { 12, "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://5054663650", Parent = { 11 }, Position = UDim2.new(0, 3, 0, 3), Size = UDim2.new(0, 10, 0, 10), } },
                        { 13, "UICorner",   { CornerRadius = UDim.new(0, 4), Parent = { 11 }, } },
                        { 14, "TextButton", { AutoButtonColor = false, BackgroundColor3 = theme.Button, BackgroundTransparency = 0.2, BorderSizePixel = 0, Font = 3, Name = "Minimize", Parent = { 8 }, Position = UDim2.new(1, -46, 0, 7), Size = UDim2.new(0, 16, 0, 16), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, } },
                        { 15, "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://5034768003", Parent = { 14 }, Position = UDim2.new(0, 3, 0, 3), Size = UDim2.new(0, 10, 0, 10), } },
                        { 16, "UICorner",   { CornerRadius = UDim.new(0, 4), Parent = { 14 }, } },
                        { 17, "ImageLabel", { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Image = "rbxassetid://1427967925", Name = "Outlines", Parent = { 2 }, Position = UDim2.new(0, -5, 0, -5), ScaleType = 1, Size = UDim2.new(1, 10, 1, 10), SliceCenter = Rect.new(6, 6, 25, 25), TileSize = UDim2.new(0, 20, 0, 20), Visible = false, } },
                        { 18, "Frame",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Name = "ResizeControls", Parent = { 2 }, Position = UDim2.new(0, -5, 0, -5), Size = UDim2.new(1, 10, 1, 10), } },
                        { 19, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "North", Parent = { 18 }, Position = UDim2.new(0, 10, 0, 5), Size = UDim2.new(1, -20, 0, 2), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
                        { 20, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "South", Parent = { 18 }, Position = UDim2.new(0, 10, 1, -7), Size = UDim2.new(1, -20, 0, 2), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
                        { 21, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "NorthEast", Parent = { 18 }, Position = UDim2.new(1, -7, 0, 5), Size = UDim2.new(0, 2, 0, 2), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
                        { 22, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "East", Parent = { 18 }, Position = UDim2.new(1, -7, 0, 10), Size = UDim2.new(0, 2, 1, -20), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
                        { 23, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "West", Parent = { 18 }, Position = UDim2.new(0, 5, 0, 10), Size = UDim2.new(0, 2, 1, -20), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
                        { 24, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "SouthEast", Parent = { 18 }, Position = UDim2.new(1, -7, 1, -7), Size = UDim2.new(0, 2, 0, 2), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
                        { 25, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "NorthWest", Parent = { 18 }, Position = UDim2.new(0, 5, 0, 5), Size = UDim2.new(0, 2, 0, 2), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
                        { 26, "TextButton", { AutoButtonColor = false, BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = 3, Name = "SouthWest", Parent = { 18 }, Position = UDim2.new(0, 5, 1, -7), Size = UDim2.new(0, 2, 0, 2), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
                        { 27, "Frame",      { BackgroundTransparency = 1, Name = "BottomDrag", Parent = { 2 }, Position = UDim2.new(0.5, -70, 1, 8), Size = UDim2.new(0, 140, 0, 16) } },
                        { 28, "Frame",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 0.8, Name = "Bar", Parent = { 27 }, Position = UDim2.new(0.5, -60, 0.5, -2), Size = UDim2.new(0, 120, 0, 4), AnchorPoint = Vector2.new(0, 0) } },
                        { 29, "UICorner",   { CornerRadius = UDim.new(1, 0), Parent = { 28 }, } },
                        { 30, "Frame",      { BackgroundTransparency = 1, Name = "CornerScale", Parent = { 2 }, Position = UDim2.new(1, 0, 1, 0), Size = UDim2.new(0, 30, 0, 30), AnchorPoint = Vector2.new(0.5, 0.5) } },
                        { 31, "Frame",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 0.8, Name = "Bar", Parent = { 30 }, Position = UDim2.new(0, 8, 1, -4), Size = UDim2.new(0, 18, 0, 3) } },
                        { 32, "UICorner",   { CornerRadius = UDim.new(1, 0), Parent = { 31 }, } },
                        { 33, "Frame",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 0.8, Name = "Bar2", Parent = { 30 }, Position = UDim2.new(1, -4, 0, 8), Size = UDim2.new(0, 3, 0, 18) } },
                        { 34, "UICorner",   { CornerRadius = UDim.new(1, 0), Parent = { 33 }, } },
                    })
                    ...
                    self.GuiElems.BottomDrag = guiMain.BottomDrag
                    self.GuiElems.CornerScale = guiMain.CornerScale
                    self.GuiElems.ResizeControls = guiResizeControls
                    self.ContentPane = guiMain.Content

                    local function setupHandleEffect(handle)
                        local bar = handle:FindFirstChild("Bar")
                        local bar2 = handle:FindFirstChild("Bar2")
                        if not bar then return end
                        handle.InputBegan:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.MouseMovement then
                                service.TweenService:Create(bar, TweenInfo.new(0.1), { BackgroundTransparency = 0.2 }):Play()
                                if bar2 then service.TweenService:Create(bar2, TweenInfo.new(0.1), { BackgroundTransparency = 0.2 }):Play() end
                            end
                        end)
                        handle.InputEnded:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.MouseMovement then
                                service.TweenService:Create(bar, TweenInfo.new(0.1), { BackgroundTransparency = 0.8 }):Play()
                                if bar2 then service.TweenService:Create(bar2, TweenInfo.new(0.1), { BackgroundTransparency = 0.8 }):Play() end
                            end
                        end)
                    end

                    setupHandleEffect(self.GuiElems.BottomDrag)
                    setupHandleEffect(self.GuiElems.CornerScale)

                    local function handleDrag(handle)
                        handle.InputBegan:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.MouseButton1 and self.Draggable then
                                ...
                                releaseEvent = cloneref(game["Run Service"].Parent:GetService("UserInputService"))
                                .InputEnded:Connect(function(input)
                                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                                        ...
                                        if alignInsertSide then
                                            local targetSide = (alignInsertSide == "left" and leftSide) or
                                ...
                                mouseEvent = cloneref(game["Run Service"].Parent:GetService("UserInputService"))
                                            ...
                                            if self.Resizable and self.Alignable then
                                                if inputX < 25 then
                                                    if sideHasRoom(leftSide, self.MinY or 100) then
                                                        ...
                                                        alignIndicator.Indicator.Size = UDim2.new(0, 40, 0, range[2] -
                                                        range[1])
                    ...
                    handleDrag(guiTopBar)
                    handleDrag(self.GuiElems.BottomDrag)

                    resizeHook(self, self.GuiElems.CornerScale, "SE")
                    ...
                    for i, v in pairs(side.Windows) do
                        ...
                        if not isEnd then
                            ...
                            side.ResizeCons[#side.ResizeCons + 1] = v.Gui.Main:GetPropertyChangedSignal("Size"):Connect(function()
                                newTemplate.Position = UDim2.new(1, -side.Width, 0,
                                    v.GuiElems.Main.Position.Y.Offset + v.GuiElems.Main.Size.Y.Offset)
                            end)
                            side.ResizeCons[#side.ResizeCons + 1] = v.Gui.Main:GetPropertyChangedSignal("Position")
                            :Connect(function()
                                newTemplate.Position = UDim2.new(1, -side.Width, 0,
                                    v.GuiElems.Main.Position.Y.Offset + v.GuiElems.Main.Size.Y.Offset)
                            end)
                ...
                funcs.SetMinimized = function(self, set, mode)
                    ...
                    if mode == 1 or mode == 2 then
                        ...
                        if mode == 1 then
                            ...
                            local newPos = UDim2.new(0, self.PosX, 0,
                                newVal and math.min(maxY - 20, self.PosY + self.SizeY - 20) or
                        ...
                        self.GuiElems.Minimize.ImageLabel.Image = newVal and "rbxassetid://5060023708" or
            ...
            Lib.ContextMenu = (function()
                ...
                local function createGui(self)
                    local contextGui = create({
                        ...
                        { 2,  "Frame",          { Active = true, BackgroundColor3 = Color3.fromRGB(20, 20, 20), BorderSizePixel = 0, Name = "Main", Parent = { 1 }, Position = UDim2.new(0.5, -100, 0.5, -150), Size = UDim2.new(0, 200, 0, 100), } },
                        { 3,  "UICorner",       { CornerRadius = UDim.new(0, 8), Parent = { 2 }, } },
                        { 4,  "Frame",          { BackgroundColor3 = Color3.fromRGB(30, 30, 30), BorderSizePixel = 0, Name = "Container", Parent = { 2 }, Position = UDim2.new(0, 1, 0, 1), Size = UDim2.new(1, -2, 1, -2), } },
                        { 5,  "UICorner",       { CornerRadius = UDim.new(0, 8), Parent = { 4 }, } },
                        { 6,  "ScrollingFrame", { Active = true, BackgroundColor3 = Color3.new(0, 0, 0), BackgroundTransparency = 1, BorderSizePixel = 0, CanvasSize = UDim2.new(0, 0, 0, 0), Name = "List", Parent = { 4 }, Position = UDim2.new(0, 4, 0, 4), ScrollBarImageColor3 = Color3.new(0, 0, 0), ScrollBarThickness = 0, Size = UDim2.new(1, -8, 1, -8), VerticalScrollBarInset = 0, } },
                        { 7,  "UIListLayout",   { Parent = { 6 }, SortOrder = 2, Padding = UDim.new(0, 2) } },
                        { 8,  "Frame",          { BackgroundColor3 = Color3.fromRGB(35, 35, 35), BorderSizePixel = 0, Name = "SearchFrame", Parent = { 4 }, Size = UDim2.new(1, 0, 0, 32), Visible = false, } },
                        { 9,  "Frame",          { BackgroundColor3 = Color3.fromRGB(25, 25, 25), BorderSizePixel = 0, Name = "SearchContainer", Parent = { 8 }, Position = UDim2.new(0, 4, 0, 4), Size = UDim2.new(1, -8, 0, 24), } },
                        { 10, "UICorner",       { CornerRadius = UDim.new(0, 6), Parent = { 9 }, } },
                        { 11, "TextBox",        { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "SearchBox", Parent = { 9 }, PlaceholderColor3 = Color3.fromRGB(150, 150, 150), PlaceholderText = "Search", Position = UDim2.new(0, 8, 0, 0), Size = UDim2.new(1, -16, 0, 24), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 13, TextXAlignment = 0, } },
                        { 12, "Frame",          { BackgroundColor3 = Color3.fromRGB(45, 45, 45), BorderSizePixel = 0, Name = "Line", Parent = { 8 }, Position = UDim2.new(0, 0, 1, 0), Size = UDim2.new(1, 0, 0, 1), } },
                        { 13, "TextButton",     { AutoButtonColor = false, BackgroundColor3 = Color3.fromRGB(60, 60, 60), BackgroundTransparency = 1, BorderSizePixel = 0, Font = Enum.Font.Gotham, Name = "Entry", Parent = { 1 }, Size = UDim2.new(1, 0, 0, 26), Text = "", TextSize = 13, Visible = false, } },
                        { 14, "UICorner",       { CornerRadius = UDim.new(0, 6), Parent = { 13 }, } },
                        { 15, "TextLabel",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = Enum.Font.Gotham, Name = "EntryName", Parent = { 13 }, Position = UDim2.new(0, 28, 0, 0), Size = UDim2.new(1, -32, 1, 0), Text = "Duplicate", TextColor3 = Color3.fromRGB(230, 230, 230), TextSize = 13, TextXAlignment = 0, } },
                        { 16, "TextLabel",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "Shortcut", Parent = { 13 }, Position = UDim2.new(0, 28, 0, 0), Size = UDim2.new(1, -36, 1, 0), Text = "Ctrl+D", TextColor3 = Color3.fromRGB(160, 160, 160), TextSize = 12, TextXAlignment = 1, } },
                        { 17, "ImageLabel",     { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, ImageRectOffset = Vector2.new(304, 0), ImageRectSize = Vector2.new(16, 16), Name = "Icon", Parent = { 13 }, Position = UDim2.new(0, 6, 0, 5), ScaleType = 4, Size = UDim2.new(0, 16, 0, 16), } },
                        { 18, "Frame",          { BackgroundColor3 = Color3.new(0, 0, 0), BackgroundTransparency = 1, BorderSizePixel = 0, Name = "Divider", Parent = { 1 }, Position = UDim2.new(0, 0, 0, 20), Size = UDim2.new(1, 0, 0, 12), Visible = false, } },
                        { 19, "Frame",          { BackgroundColor3 = Color3.fromRGB(45, 45, 45), BorderSizePixel = 0, Name = "Line", Parent = { 18 }, Position = UDim2.new(0, 4, 0.5, 0), Size = UDim2.new(1, -8, 0, 1), } },
                        { 20, "TextLabel",      { AnchorPoint = Vector2.new(0, 0.5), BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = Enum.Font.GothamBold, Name = "DividerName", Parent = { 18 }, Position = UDim2.new(0, 8, 0.5, 0), Size = UDim2.new(1, -16, 1, 0), Text = "Objects", TextColor3 = Color3.fromRGB(120, 120, 120), TextSize = 11, TextTransparency = 0, TextXAlignment = 0, Visible = false, } },
                    })
                ...
                funcs.AddDivider = function(self, text)
                    ...
                    service.TextService:GetTextSize(text, 14, Enum.Font.SourceSans, Vector2.new(999999999, 20)).X or nil
                ...
                funcs.Refresh = function(self)
                    ...
                    for i = 1, #items do
                        ...
                        if item.Divider then
                            ...
                            newEntry.InputBegan:Connect(function(input)
                                if input.UserInputType == Enum.UserInputType.MouseMovement then
                                    service.TweenService:Create(newEntry, TweenInfo.new(0.1),
            ...
            Lib.CodeFrame = (function()
                ...
                local function setupMouseSelection(obj)
                    ...
                    codeFrame.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.MouseButton1 then
                            ...
                            scrollEvent = cloneref(game["Run Service"].Parent:GetService("RunService")).RenderStepped
                            :Connect(function()
                ...
                local function makeFrame(obj)
                    local frame = create({
                        { 1, "Frame",    { BackgroundColor3 = Color3.fromRGB(30, 30, 30), BorderSizePixel = 0, Position = UDim2.new(0.5, -300, 0.5, -200), Size = UDim2.new(0, 600, 0, 400), } },
                        { 2, "UICorner", { CornerRadius = UDim.new(0, 8), Parent = { 1 }, } },
                    })
                    local elems = {}
                    ...
                    lineNumbersLabel.Font = Enum.Font.Gotham
                    ...
                    lineTweens.Invis = tweenService:Create(cursor,
                        TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
                        { BackgroundTransparency = 1 })
                    lineTweens.Vis = tweenService:Create(cursor,
                    ...
                    elems.ScrollCorner = create({
                        { 1, "Frame",    { BackgroundColor3 = Color3.fromRGB(30, 30, 30), BorderSizePixel = 0, Name = "ScrollCorner", Position = UDim2.new(1, -10, 1, -10), Size = UDim2.new(0, 10, 0, 10), Visible = false, } },
                        { 2, "UICorner", { CornerRadius = UDim.new(0, 4), Parent = { 1 }, } },
                    })
                ...
                funcs.TabAdjust = function(self, x, y)
                    ...
                    if line then
                        ...
                        (#middle > 0 and middle or " ") .. (#right > 0 and right or " ")
                            ...
                            if find(char, "[%a_]") then
                                ...
                                if wordType ~= 7 then
                                    if wordBeginsDotted then
                                        ...
                                        wordType = (prevBuiltIn and type(prevBuiltIn) == "table" and prevBuiltIn[word] and 8) or
                                        10
                                    end
                    ...
                    for row = 1, maxLines do
                        ...
                        for col = 2, maxCols do
                            ...
                            if posType ~= curType then
                                local template = (inSelection and selectionTemplate) or richTemplates[typeMap[posType]] or
                                textTemplate
            ...
            Lib.Checkbox = (function()
                ...
                funcs.SetState = function(self, val, anim)
                    ...
                    if self.Toggled then
                        if self.Style == 0 then
                            if anim then
                                self.OutlineColorTween = service.TweenService:Create(self.GuiElems.Outline,
            ...
            Lib.ColorPicker = (function() -- TODO: Convert to newer class model
                ...
                local function new()
                    ...
                    local basicColors = { Color3.new(0, 0, 0), Color3.new(0.66666668653488, 0, 0), Color3.new(0,
                        0.33333334326744, 0), Color3.new(0.66666668653488, 0.33333334326744, 0), Color3.new(0,
                        0.66666668653488, 0), Color3.new(0.66666668653488, 0.66666668653488, 0), Color3.new(0, 1, 0),
                        Color3.new(0.66666668653488, 1, 0), Color3.new(0, 0, 0.49803924560547), Color3.new(
                    0.66666668653488, 0, 0.49803924560547), Color3.new(0, 0.33333334326744, 0.49803924560547), Color3
                        .new(0.66666668653488, 0.33333334326744, 0.49803924560547), Color3.new(0, 0.66666668653488,
                        0.49803924560547), Color3.new(0.66666668653488, 0.66666668653488, 0.49803924560547), Color3.new(
                    0, 1, 0.49803924560547), Color3.new(0.66666668653488, 1, 0.49803924560547), Color3.new(0, 0, 1),
                        Color3.new(0.66666668653488, 0, 1), Color3.new(0, 0.33333334326744, 1), Color3.new(
                    0.66666668653488, 0.33333334326744, 1), Color3.new(0, 0.66666668653488, 1), Color3.new(
                    0.66666668653488, 0.66666668653488, 1), Color3.new(0, 1, 1), Color3.new(0.66666668653488, 1, 1),
                        Color3.new(0.33333334326744, 0, 0), Color3.new(1, 0, 0), Color3.new(0.33333334326744,
                        0.33333334326744, 0), Color3.new(1, 0.33333334326744, 0), Color3.new(0.33333334326744,
                        0.66666668653488, 0), Color3.new(1, 0.66666668653488, 0), Color3.new(0.33333334326744, 1, 0),
                        Color3.new(1, 1, 0), Color3.new(0.33333334326744, 0, 0.49803924560547), Color3.new(1, 0,
                        0.49803924560547), Color3.new(0.33333334326744, 0.33333334326744, 0.49803924560547), Color3.new(
                    1, 0.33333334326744, 0.49803924560547), Color3.new(0.33333334326744, 0.66666668653488,
                        0.49803924560547), Color3.new(1, 0.66666668653488, 0.49803924560547), Color3.new(
                    0.33333334326744, 1, 0.49803924560547), Color3.new(1, 1, 0.49803924560547), Color3.new(
                    0.33333334326744, 0, 1), Color3.new(1, 0, 1), Color3.new(0.33333334326744, 0.33333334326744, 1),
                        Color3.new(1, 0.33333334326744, 1), Color3.new(0.33333334326744, 0.66666668653488, 1), Color3
                        .new(1, 0.66666668653488, 1), Color3.new(0.33333334326744, 1, 1), Color3.new(1, 1, 1) }
                    local customColors = {}
            ...
            Lib.NumberSequenceEditor = (function()
                local function new() -- TODO: Convert to newer class model
                    ...
                    envelopeDragBottom.InputBegan:Connect(function(input)
                        ...
                        mouseEvent = user.InputChanged:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.MouseMovement then
                                local bottomDiff = (mouse.Y + (20 - mouseDelta)) - (currentPoint[4].AbsolutePosition.Y + 2) -
                                19
                ...
                local function new()
                    ...
                    colorLine.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.MouseButton1 and #colors < 20 then
                            ...
                            local lerpColor = fromColor[1]:lerp(toColor[1], (raw - fromColor[2]) /
            ...
            Lib.Button = (function()
                ...
                local function new()
                    ...
                    obj.Anim = Lib.ButtonAnim(b,
                        { Mode = 2, StartColor = Settings.Theme.Button, HoverColor = Settings.Theme.ButtonHover, PressColor =
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, AI, DefaultSettings, Notebook, Serializer, Lib
local API, RMD
...
Main = (function()
    ...
    Main.LoadAISettings = function()
        if not env.readfile then return end
        local ok, data = pcall(env.readfile, "dex/ai_settings.json")
        if ok and data and data ~= "" then
            local okDecode, decoded = pcall(service.HttpService.JSONDecode, service.HttpService, data)
            if okDecode and decoded then
                Main.AISettings.ApiKey = decoded.ApiKey or ""
                Main.AISettings.Model = decoded.Model or Main.AISettings.Model
                Main.AISettings.MaxOutputTokens = decoded.MaxOutputTokens or Main.AISettings.MaxOutputTokens
                Main.AISettings.MaxContextMessages = decoded.MaxContextMessages or Main.AISettings.MaxContextMessages
                Main.AISettings.PersistChat = decoded.PersistChat or false
            end
        end
    end

    Main.SaveAISettings = function()
        if not env.writefile then return end
        local payload = {
            ApiKey = Main.AISettings.ApiKey or "",
            Model = Main.AISettings.Model or "gemini-3-flash-preview",
            MaxOutputTokens = Main.AISettings.MaxOutputTokens or 8192,
            MaxContextMessages = Main.AISettings.MaxContextMessages or 60,
            PersistChat = Main.AISettings.PersistChat == true
        }
        env.writefile("dex/ai_settings.json", service.HttpService:JSONEncode(payload))
    end
    ...
    Main.CreateIntro = function(initStatus)
        local gui = create({
            ...
            { 2,  "Frame",      { Active = true, BackgroundColor3 = Color3.fromRGB(20, 20, 20), BorderSizePixel = 0, Name = "Main", Parent = { 1 }, Position = UDim2.new(0.5, -175, 0.5, -100), Size = UDim2.new(0, 350, 0, 200), } },
            { 3,  "UICorner",   { CornerRadius = UDim.new(0, 12), Parent = { 2 }, } },
            { 4,  "Frame",      { BackgroundColor3 = Color3.fromRGB(30, 30, 30), BorderSizePixel = 0, ClipsDescendants = true, Name = "Holder", Parent = { 2 }, Size = UDim2.new(1, 0, 1, 0), } },
            { 5,  "UICorner",   { CornerRadius = UDim.new(0, 12), Parent = { 4 }, } },
            { 6,  "UIGradient", { Parent = { 4 }, Rotation = 30, Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 1, 0), NumberSequenceKeypoint.new(1, 1, 0), }), } },
            { 7,  "TextLabel",  { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.GothamMedium, Name = "Title", Parent = { 4 }, Position = UDim2.new(0, -190, 0, 15), Size = UDim2.new(0, 100, 0, 50), Text = "Dex", TextColor3 = Color3.new(1, 1, 1), TextSize = 50, TextTransparency = 1, } },
            { 8,  "TextLabel",  { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "Desc", Parent = { 4 }, Position = UDim2.new(0, -230, 0, 60), Size = UDim2.new(0, 180, 0, 25), Text = "Ultimate Debugging Suite", TextColor3 = Color3.fromRGB(200, 200, 200), TextSize = 18, TextTransparency = 1, } },
            { 9,  "TextLabel",  { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "StatusText", Parent = { 4 }, Position = UDim2.new(0, 20, 0, 110), Size = UDim2.new(0, 180, 0, 25), Text = "Fetching API", TextColor3 = Color3.fromRGB(180, 180, 180), TextSize = 14, TextTransparency = 1, } },
            { 10, "Frame",      { BackgroundColor3 = Color3.fromRGB(40, 40, 40), BorderSizePixel = 0, Name = "ProgressBar", Parent = { 4 }, Position = UDim2.new(0, 110, 0, 145), Size = UDim2.new(0, 0, 0, 4), } },
            ...
            { 18, "Frame",      { BackgroundTransparency = 1, BorderSizePixel = 0, Name = "Outlines", Parent = { 2 }, Size = UDim2.new(1, 0, 1, 0), } },
            { 19, "UICorner",   { CornerRadius = UDim.new(0, 12), Parent = { 18 }, } },
            { 20, "UIStroke",   { Color = Color3.fromRGB(0, 162, 255), Thickness = 1, Parent = { 18 }, } },
            { 21, "UIGradient", { Parent = { 18 }, Rotation = 30, Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 1, 0), NumberSequenceKeypoint.new(1, 1, 0), }), } },
            { 22, "UIGradient", { Parent = { 2 }, Rotation = 30, Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 1, 0), NumberSequenceKeypoint.new(1, 1, 0), }), } },
            { 23, "ImageLabel", { BackgroundTransparency = 1, Image = "rbxassetid://5034768003", Name = "ImageLabel", Parent = { 10 }, Position = UDim2.new(1, 5, 0.5, -8), Size = UDim2.new(0, 16, 0, 16), ImageTransparency = 1, } },
        })
        Main.ShowGui(gui)
        ...
        tweenS:Create(progressBar, progressTI, { Position = UDim2.new(0, 60, 0, 145), Size = UDim2.new(0, 100, 0, 4) })
            :Play()
    ...
    Main.CreateApp = function(data)
        ...
        app.Main.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseMovement then
                ...
                app.Main.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
            end
        end)
        ...
        app.Main.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseMovement then
                ...
                app.Main.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
            end
        end)
    ...
    Main.SetMainGuiOpen = function(val)
        ...
        service.TweenService:Create(Main.MainGui.OpenButton,
            ...
            { BackgroundTransparency = val and 0 or (Lib.CheckMouseInGui(Main.MainGui.OpenButton) and 0 or 0.2) }):Play()
    ...
    Main.ShowSettingsWindow = function()
        if not Lib then return end
        if not Main.SettingsWindow then
            local window = Lib.Window.new()
            ...
            local keyLabel = createSimple("TextLabel", {
                Parent = content,
                BackgroundTransparency = 1,
                Font = Enum.Font.Gotham,
            ...
            local keyBox = createSimple("TextBox", {
                Parent = content,
            ...
            Lib.ViewportTextBox.convert(keyBox)

            local modelLabel = createSimple("TextLabel", {
                Parent = content,
                BackgroundTransparency = 1,
                Font = Enum.Font.Gotham,
            ...
            local modelBox = createSimple("TextBox", {
                Parent = content,
                BackgroundColor3 = Settings.Theme.TextBox,
                BorderColor3 = Settings.Theme.Outline3,
                ClearTextOnFocus = false,
            ...
            Lib.ViewportTextBox.convert(modelBox)

            local tokenLabel = createSimple("TextLabel", {
                Parent = content,
                BackgroundTransparency = 1,
                Font = Enum.Font.Gotham,
            ...
            local tokenBox = createSimple("TextBox", {
                Parent = content,
                BackgroundColor3 = Settings.Theme.TextBox,
                BorderColor3 = Settings.Theme.Outline3,
                ClearTextOnFocus = false,
            ...
            Lib.ViewportTextBox.convert(tokenBox)

            local contextLabel = createSimple("TextLabel", {
                Parent = content,
                BackgroundTransparency = 1,
                Font = Enum.Font.Gotham,
            ...
            local contextBox = createSimple("TextBox", {
                Parent = content,
                BackgroundColor3 = Settings.Theme.TextBox,
                BorderColor3 = Settings.Theme.Outline3,
                ClearTextOnFocus = false,
            ...
            Lib.ViewportTextBox.convert(contextBox)

            local persistCheckbox = Lib.Checkbox.new(1)
            persistCheckbox.Gui.Parent = content
            persistCheckbox.Gui.Position = UDim2.new(0, 12, 0, 234)
            local persistLabel = createSimple("TextLabel", {
                Parent = content,
                BackgroundTransparency = 1,
                Font = Enum.Font.Gotham,
                Position = UDim2.new(0, 36, 0, 232),
                Size = UDim2.new(1, -48, 0, 20),
                Text = "Persist chat history",
                TextColor3 = Settings.Theme.Text,
                TextSize = 12,
                TextXAlignment = Enum.TextXAlignment.Left
            })

            local saveButton = createSimple("TextButton", {
                Parent = content,
                BackgroundColor3 = Settings.Theme.Button,
                BorderSizePixel = 0,
                Font = Enum.Font.GothamMedium,
            ...
            createSimple("UICorner", { CornerRadius = UDim.new(0, 6), Parent = saveButton })
            Lib.ButtonAnim(saveButton,
                { Mode = 2, StartColor = Settings.Theme.Button, LerpTo = Settings.Theme.ButtonHover, LerpDelta = 0.2 })
            ...
            local status = createSimple("TextLabel", {
                ...
                BackgroundTransparency = 1,
                Font = Enum.Font.Gotham,
                Position = UDim2.new(0, 100, 0, 266),
            ...
            saveButton.MouseButton1Click:Connect(function()
                Main.AISettings.ApiKey = keyBox.Text or ""
                Main.AISettings.Model = (modelBox.Text ~= "" and modelBox.Text) or Main.AISettings.Model
                local tok = tonumber(tokenBox.Text)
                if tok then Main.AISettings.MaxOutputTokens = tok end
                local ctx = tonumber(contextBox.Text)
                if ctx then Main.AISettings.MaxContextMessages = ctx end
                Main.AISettings.PersistChat = persistCheckbox.Toggled == true
                Main.SaveAISettings()
                status.Text = "Saved"
            end)

            window.OnActivate:Connect(function()
                keyBox.Text = Main.AISettings.ApiKey or ""
                modelBox.Text = Main.AISettings.Model or "gemini-3-flash-preview"
                tokenBox.Text = tostring(Main.AISettings.MaxOutputTokens or 8192)
                contextBox.Text = tostring(Main.AISettings.MaxContextMessages or 60)
                persistCheckbox:SetState(Main.AISettings.PersistChat == true, false)
                status.Text = ""
            end)

            Main.SettingsWindow = window
            Main.SettingsWindowElems = {
                KeyBox = keyBox,
                ModelBox = modelBox,
                TokenBox = tokenBox,
        ...
        local elems = Main.SettingsWindowElems
        if elems then
            elems.KeyBox.Text = Main.AISettings.ApiKey or ""
            elems.ModelBox.Text = Main.AISettings.Model or "gemini-3-flash-preview"
            elems.TokenBox.Text = tostring(Main.AISettings.MaxOutputTokens or 8192)
            elems.ContextBox.Text = tostring(Main.AISettings.MaxContextMessages or 60)
            if elems.PersistCheckbox then
                elems.PersistCheckbox:SetState(Main.AISettings.PersistChat == true, false)
            end
            elems.Status.Text = ""
        end
        Main.SettingsWindow:ShowAndFocus()
    end

    Main.ShowInfoWindow = function()
        if not Lib then return end
        if not Main.InfoWindow then
            local window = Lib.Window.new()
            ...
            local infoText = table.concat({
                "Dex AI lets Gemini read and query live game data.",
                "",
                "Set your API key in Settings (gear icon).",
                "",
                "Tool actions:",
                "search, select, selection, info, properties, decompile, remotes, path, children",
                "",
                "Example:",
                "@DEX {\"action\":\"search\",\"query\":\"Player\",\"class\":\"Folder\",\"limit\":10}"
            }, "\n")
            ...
            Main.InfoWindow = window
        end

        Main.InfoWindow:ShowAndFocus()
    end

    Main.CreateMainGui = function()
        local gui = create({
            { 1,  "ScreenGui",      { IgnoreGuiInset = true, Name = "MainMenu", } },
            { 2,  "TextButton",     { AnchorPoint = Vector2.new(0.5, 0), AutoButtonColor = false, BackgroundColor3 = Color3.fromRGB(25, 25, 25), BorderSizePixel = 0, Font = Enum.Font.GothamMedium, Name = "OpenButton", Parent = { 1 }, Position = UDim2.new(0.5, 0, 0, 10), Size = UDim2.new(0, 40, 0, 40), Text = "Dex", TextColor3 = Color3.new(1, 1, 1), TextSize = 16, TextTransparency = 0.2, } },
            { 3,  "UICorner",       { CornerRadius = UDim.new(0, 8), Parent = { 2 }, } },
            { 4,  "Frame",          { AnchorPoint = Vector2.new(0.5, 0), BackgroundColor3 = Color3.fromRGB(20, 20, 20), ClipsDescendants = true, Name = "MainFrame", Parent = { 2 }, Position = UDim2.new(0.5, 0, 1, 10), Size = UDim2.new(0, 224, 0, 200), } },
            { 5,  "UICorner",       { CornerRadius = UDim.new(0, 10), Parent = { 4 }, } },
            ...
            { 7,  "UICorner",       { CornerRadius = UDim.new(0, 10), Parent = { 6 }, } },
            { 8,  "Frame",          { BackgroundColor3 = Color3.fromRGB(30, 30, 30), BorderSizePixel = 0, Name = "CoverFrame", Parent = { 6 }, Size = UDim2.new(1, 0, 0, 4), } },
            { 9,  "Frame",          { BackgroundColor3 = Color3.fromRGB(40, 40, 40), BorderSizePixel = 0, Name = "Line", Parent = { 8 }, Position = UDim2.new(0, 0, 0, -1), Size = UDim2.new(1, 0, 0, 1), } },
            { 10, "TextButton",     { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "Settings", Parent = { 6 }, Position = UDim2.new(1, -60, 0, 0), Size = UDim2.new(0, 28, 1, 0), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, } },
            { 11, "ImageLabel",     { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://6578871732", ImageTransparency = 0.2, Name = "Icon", Parent = { 10 }, Position = UDim2.new(0, 6, 0, 6), Size = UDim2.new(0, 16, 0, 16), } },
            { 12, "TextButton",     { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Font = Enum.Font.Gotham, Name = "Information", Parent = { 6 }, Position = UDim2.new(1, -32, 0, 0), Size = UDim2.new(0, 28, 1, 0), Text = "", TextColor3 = Color3.new(1, 1, 1), TextSize = 14, } },
            { 13, "ImageLabel",     { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://6578933307", ImageTransparency = 0.2, Name = "Icon", Parent = { 12 }, Position = UDim2.new(0, 6, 0, 6), Size = UDim2.new(0, 16, 0, 16), } },
            { 14, "ScrollingFrame", { Active = true, AnchorPoint = Vector2.new(0.5, 0), BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderColor3 = Color3.new(0.1294117718935, 0.1294117718935, 0.1294117718935), BorderSizePixel = 0, Name = "AppsFrame", Parent = { 4 }, Position = UDim2.new(0.5, 0, 0, 0), ScrollBarImageColor3 = Color3.new(0, 0, 0), ScrollBarThickness = 0, Size = UDim2.new(0, 222, 1, -29), } },
            ...
            { 16, "UIGridLayout",   { CellSize = UDim2.new(0, 66, 0, 80), Parent = { 15 }, SortOrder = 2, } },
            ...
            { 18, "TextButton",     { AutoButtonColor = false, BackgroundColor3 = Color3.fromRGB(45, 45, 45), BorderSizePixel = 0, Font = Enum.Font.Gotham, Name = "Main", Parent = { 17 }, Size = UDim2.new(1, 0, 0, 64), Text = "", TextColor3 = Color3.new(0, 0, 0), TextSize = 14, } },
            { 19, "UICorner",       { CornerRadius = UDim.new(0, 8), Parent = { 18 }, } },
            { 20, "ImageLabel",     { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, Image = "rbxassetid://6579106223", ImageRectSize = Vector2.new(32, 32), Name = "Icon", Parent = { 18 }, Position = UDim2.new(0.5, -16, 0, 8), ScaleType = 4, Size = UDim2.new(0, 32, 0, 32), } },
            { 21, "TextLabel",      { BackgroundColor3 = Color3.new(1, 1, 1), BackgroundTransparency = 1, BorderSizePixel = 0, Font = Enum.Font.Gotham, Name = "AppName", Parent = { 18 }, Position = UDim2.new(0, 2, 0, 44), Size = UDim2.new(1, -4, 1, -44), Text = "Explorer", TextColor3 = Color3.fromRGB(240, 240, 240), TextSize = 12, TextTransparency = 0.1, TextTruncate = 1, TextWrapped = true, TextYAlignment = 0, } },
            { 22, "Frame",          { BackgroundColor3 = Color3.fromRGB(0, 162, 255), BorderSizePixel = 0, Name = "Highlight", Parent = { 18 }, Position = UDim2.new(0.5, -15, 1, -4), Size = UDim2.new(0, 30, 0, 2), } },
            { 23, "UICorner",       { CornerRadius = UDim.new(1, 0), Parent = { 22 }, } },
        })
        Main.MainGui = gui
        ...
        openButton.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseMovement then
                service.TweenService:Create(Main.MainGui.OpenButton,
                    TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = 0 })
                    :Play()
            end
        end)
        ...
        local infoButton = gui.OpenButton.MainFrame.BottomFrame.Information
        ...
        -- Create Main Apps
        Main.CreateApp({ Name = "Explorer", IconMap = Main.LargeIcons, Icon = "Explorer", Open = true, Window = Explorer
        .Window })

        Main.CreateApp({ Name = "Properties", IconMap = Main.LargeIcons, Icon = "Properties", Open = true, Window =
        Properties.Window })

        Main.CreateApp({ Name = "Script Viewer", IconMap = Main.LargeIcons, Icon = "Script_Viewer", Window = ScriptViewer
        .Window })

        if AI and AI.Window then
            Main.CreateApp({ Name = "AI", Icon = "rbxassetid://119604644446321", Window = AI.Window })
        end
    ...
    Main.Init = function()
        Main.Elevated = pcall(function() local a = cloneref(game["Run Service"].Parent:GetService("CoreGui"))
            :GetFullName() end)
        Main.InitEnv()
        Main.LoadSettings()
        Main.SetupFilesystem()
        Main.LoadAISettings()
        if EmbeddedModules and EmbeddedModules.AI then
            Main.ModuleList[#Main.ModuleList + 1] = "AI"
        else
            local modFolder = script and script:FindFirstChild("Modules")
            if modFolder and modFolder:FindFirstChild("AI") then
                Main.ModuleList[#Main.ModuleList + 1] = "AI"
            end
        end
        ...
        Main.AIIcon = Lib.IconMap.newLinear("rbxassetid://119604644446321", 32, 32)
        ...
        Explorer.Window:Resize(400, 600)
        Explorer.Window:Show({ Silent = true })
        Lib.FastWait()
        Explorer.Window.GuiElems.Main.Position = UDim2.new(0.5, -420, 0.5, -300)

        Properties.Window:Resize(400, 600)
        Properties.Window:Show({ Silent = true })
        Lib.FastWait()
        Properties.Window.GuiElems.Main.Position = UDim2.new(0.5, 20, 0.5, -300)
